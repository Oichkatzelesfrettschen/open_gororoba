...
impl DepsNotReplaced for Metadata {
    ...
    {
        let package = package_id.to_cargo_metadata_package(self)?;
        let node = self
            .resolve
            .as_ref()?
            .nodes
        ...
        Some(
            node.deps
                ...
                .filter(|dependency| seen.insert(dependency.pkg.clone()))
                .map(|dependency| (dependency.pkg.clone(), HashSet::new()))
                .collect(),
        )
    }
}
...
mod metadata_tests {
    ...
    #[rstest]
    fn dependencies_follow_resolved_edges_instead_of_manifest_candidates() {
        let (_, mut metadata) = construct_krates_and_metadata();
        ...
        let selected = node.deps.pop().unwrap().pkg;
        node.deps.retain(|dependency| dependency.pkg != selected);
        let dependencies = metadata.deps_not_replaced(&root, true).unwrap();
        assert!(metadata
            .packages
            .iter()
            .any(|package| package.id == selected));
        assert!(dependencies.iter().all(|(package, _)| *package != selected));
    }
