use super::metadata::package_id::ToCargoMetadataPackage;
...
impl ToCargoGeigerSource for CargoMetadataPackageId {
    fn to_cargo_geiger_source(
        ...
        match package.source {
            ...
            None => CargoGeigerSerdeSource::Path(
                Url::from_directory_path(
                    package.manifest_path.parent().unwrap().as_std_path(),
                )
                .expect(
                    "Cargo package manifest has an absolute parent directory",
                ),
            ),
        }
    }
...
#[cfg(test)]
fn handle_path_source<T: super::metadata::package_id::GetPackageIdRepr>(
    package_id: &T,
) -> CargoGeigerSerdeSource {
...
mod geiger_tests {
    ...
    #[test]
    fn path_source_uses_manifest_directory_for_current_package_ids() {
        if cfg!(windows) {
            return;
        }
        let package_id = "path+file:///fixture/member#survey-fixture@0.1.0";
        let metadata: Metadata = serde_json::from_value(serde_json::json!({
            "packages": [{
                "name": "survey-fixture", "version": "0.1.0", "id": package_id,
                "source": null, "dependencies": [], "targets": [], "features": {},
                "manifest_path": "/fixture/member/Cargo.toml"
            }],
            "workspace_members": [package_id], "workspace_default_members": [package_id],
            "resolve": null, "target_directory": "/fixture/target",
            "workspace_root": "/fixture", "metadata": null, "version": 1
        }))
        .unwrap();
        let source = metadata.packages[0].id.to_cargo_geiger_source(&metadata);
        assert_eq!(
            source,
            CargoGeigerSerdeSource::Path(
                Url::parse("file:///fixture/member/").unwrap()
            )
        );
    }
