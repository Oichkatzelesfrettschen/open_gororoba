    ...
    F: FnMut(usize, usize) -> CargoResult<()>,
{
    ...
    // Deep generated expressions require more stack than Rayon's default worker.
    // Four workers reserve at most 64 MiB of parser stack across the pool.
    let pool = ThreadPoolBuilder::new()
        ...
        .build()
        .expect("create bounded parser worker pool");
    ...
    F: Fn(usize, usize) + Send + Sync,
{
    ...
    let parse_errors = Arc::new(Mutex::new(HashMap::new()));
    ...
    package_code_files.into_par_iter().for_each_with(
        (
            package_id_to_metrics.clone(),
            ignored.clone(),
            parse_errors.clone(),
        ),
        |(package_id_to_metrics, ignored, parse_errors),
         (package_id, rs_code_file)| {
            ...
            match find_unsafe_in_file(&path_buf, include_tests) {
                Err(error) => {
                    parse_errors
                        .lock()
                        .unwrap()
                        .insert(path_buf.clone(), error.to_string());
    ...
    GeigerContext {
        ...
        parse_errors: Arc::try_unwrap(parse_errors)
            .unwrap()
            .into_inner()
            .unwrap(),
    }
