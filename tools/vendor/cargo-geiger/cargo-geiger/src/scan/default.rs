...
pub(super) fn scan(
    ...
    let mut compile_options = build_compile_options(
        &scan_parameters.args.features_args,
        scan_parameters.gctx,
    );

    if scan_parameters.args.workspace_report {
        compile_options.spec = cargo::ops::Packages::All;
        compile_options.filter = cargo::ops::CompileFilter::Default {
