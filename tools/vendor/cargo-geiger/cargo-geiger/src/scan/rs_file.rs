...
pub fn into_target_kind(
    raw_target_kind: &[krates::cm::TargetKind],
) -> TargetKind {
...
fn clean_package_specs(
    packages: impl Iterator<Item = cargo::core::PackageId>,
) -> Vec<String> {
    packages
...
pub fn resolve_rs_file_deps(
    ...
    let packages = clean_package_specs(pkg_set.package_ids());
...
mod rs_file_tests {
    ...
    use rstest::*;
    #[test]
    fn clean_specs_expand_each_name_once_across_versions() {
        use cargo::core::{PackageId, SourceId};
        let registry = SourceId::for_registry(
            &url::Url::parse("https://example.org/index").unwrap(),
        )
        .unwrap();
        let older =
            PackageId::new("shared".into(), "1.0.0".parse().unwrap(), registry);
        let newer =
            PackageId::new("shared".into(), "2.0.0".parse().unwrap(), registry);
        let specs = clean_package_specs(vec![older, newer, older].into_iter());
        assert_eq!(specs, vec!["shared"]);
    }
