...
/// Compile the workspace once and retain a dependency report for every member.
/// Used-source counters refer to the union of workspace compilation units.
pub fn scan_workspace_report(
    args: &Args,
    metadata: &CargoMetadataParameters,
    gctx: &GlobalContext,
    workspace: &Workspace,
    ...
    let ScanDetails {
        rs_files_used,
        geiger_context,
    } = default::scan(metadata, &parameters, workspace)?;
    let rustc = gctx.load_global_rustc(Some(workspace))?;
    let mut root_reports = Vec::new();
    for root in &metadata.metadata.workspace_members {
        if !geiger_context.package_id_to_metrics.contains_key(root) {
            return Err(CliError::new(
                anyhow::anyhow!("workspace member lacks metrics: {}", root),
                1,
            ));
        }
        ...
        let mut report = cargo_geiger_serde::SafetyReport::default();
        for (package, metrics) in
            package_metrics(metadata, &geiger_context, &graph, root.clone())
        {
            match metrics {
                Some(metrics) => {
                    let entry = cargo_geiger_serde::ReportEntry {
                        package,
                        unsafety: unsafe_stats(&metrics, &rs_files_used),
                    };
        ...
        root_reports.push(serde_json::json!({ "root": root, "used_source_scope": "union of workspace compilation units; not isolated per-root usage", "report": report }));
    }
    let mut ignored_paths: Vec<_> =
        geiger_context.ignored_paths.iter().collect();
    ignored_paths.sort();
    let mut unscanned_used =
        list_files_used_but_not_scanned(&geiger_context, &rs_files_used);
    unscanned_used.sort();
    let mut parse_errors: Vec<_> = geiger_context
        .parse_errors
        .iter()
        .map(
            |(path, error)| serde_json::json!({ "path": path, "error": error }),
        )
        .collect();
    parse_errors.sort_by(|left, right| {
        left["path"].as_str().cmp(&right["path"].as_str())
    });
    let parsed_files = geiger_context
        .package_id_to_metrics
        .values()
        .flat_map(|metrics| metrics.rs_path_to_metrics.keys())
        .collect::<HashSet<_>>()
        .len();
    let mut graph_excluded_packages: Vec<_> = metadata
        .metadata
        .packages
        .iter()
        .filter(|package| {
            metadata
                .krates
                .node_for_kid(&package.id.clone().into())
                .is_none()
        })
        .map(|package| package.id.clone())
        .collect();
    graph_excluded_packages.sort_by(|left, right| left.repr.cmp(&right.repr));
    let mut excluded_compiler_sources = Vec::new();
    for package in &metadata.metadata.packages {
        if graph_excluded_packages.contains(&package.id) {
            let directory = package.manifest_path.parent().unwrap();
            for path in &rs_files_used {
                if path.starts_with(directory.as_std_path()) {
                    excluded_compiler_sources.push(serde_json::json!({
                        "package": package.id, "path": path
                    }));
                }
            }
        }
    }
    excluded_compiler_sources.sort_by_key(|entry| entry.to_string());
    Ok(serde_json::json!({

        "schema_version": 1,
        "workspace_members": metadata.metadata.workspace_members,
        "dependency_graph_scope": "Cargo resolved edges intersected with krates selected-feature package graph",
        "metadata_packages_outside_selected_graph": graph_excluded_packages,
        "excluded_graph_packages_with_compiler_sources": excluded_compiler_sources,
        "compilation_count": 1,
        "used_source_scope": "union of compiler-selected workspace compilation units; not isolated per-root usage",
        "compile_target_scope": "Cargo Check default target filter; tests are not compiled",
        "lexical_tests_included": args.include_tests,
        "features": {
            "all_features": args.features_args.all_features,
            "default_features": !args.features_args.no_default_features,
            "selected": args.features_args.features,
        },

        "ignored_paths": ignored_paths,
        "parse_errors": parse_errors,
        "syntax_scan_complete": parse_errors.is_empty(),
        "syntax_files_parsed": parsed_files,
        "syntax_files_failed": parse_errors.len(),
        "custom_build_scripts_scanned": false,
        "parser_worker_stack_bytes": 16 * 1024 * 1024,
        "parser_worker_limit": 4,
        "used_but_not_scanned_files": unscanned_used,
