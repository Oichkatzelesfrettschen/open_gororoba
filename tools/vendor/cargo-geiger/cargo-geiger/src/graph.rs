    ...
    match (krates_node_option, dep_not_replaced_option) {
        (Some(Node::Krate { krate, .. }), Some(dependencies)) => {
            for (dependency_package_id, _) in dependencies {
                // krates removes weak-only optional feature edges from metadata.
                let dependency_kid: Kid = dependency_package_id.clone().into();
                if cargo_metadata_parameters
                    .krates
                    .node_for_kid(&dependency_kid)
                    .is_none()
                {
                    continue;
                }
