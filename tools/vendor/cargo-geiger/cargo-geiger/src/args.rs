...
OPTIONS:
    --workspace-report           Compile all members once and emit JSON reports.
                                  Used counters cover the workspace compilation union.
...
impl Args {
    ...
    pub fn parse_args(
        ...
        let mut args = Args {
            workspace_report: raw_args.contains("--workspace-report"),
...
pub mod args_tests {
    ...
    fn update_config_test_verbosity(
        ...
        assert_eq!(format!("{:?}", gctx.target_dir().unwrap()), target_dir_before);
    }
