//! # Extracted text: differential_lambda_survey.pdf
//!
//! - source_root: `/home/eirikr/Documents/AGL_Library/Game_Geometry_Documentation`
//! - source_relpath: `differential_lambda_survey.pdf`
//! - source_abs: `/home/eirikr/Documents/AGL_Library/Game_Geometry_Documentation/differential_lambda_survey.pdf`
//! - detected_kind: `pdf`
...
//! - subject: ``
//! - keywords: ``
//! - creation_date: `Tue Oct  2 18:50:58 2018 PDT`
//! - mod_date: `Tue Oct  2 18:50:58 2018 PDT`
//! - encrypted: `no`
//!
//! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~text
...
//! definition no longer makes sense, as it would rely on dividing by a vector \epsilon :: Rm .
//! This difficulty of differentiation with non-scalar domains is usually addressed with the notion of "partial
//! derivatives" with respect to the m scalar components of the domain Rm , often written "\partialf /\partialxj " for j ∈ {1, . . . , m}.
//! When the codomain Rn is also non-scalar (i.e., n > 1), we have a matrix J (the Jacobian), with Jij = \partialfi /\partialxj
...
//!
//! Equivalently,
//! f (x + \epsilon) - (f x + \epsilon * f 0 x )
//! =0
//! \epsilon->0
//! \epsilon
//! lim
//!
//! Notice that f 0 x is used to linearly transform \epsilon. Next, generalize this condition to say that f 0 x is a linear map
//! such that
//! kf (x + \epsilon) - (f x + f 0 x \epsilon)k
//! = 0.
//! lim
//! \epsilon->0
//! k\epsilonk
...
//! D0+ :: (a -> b) -> ((a -> b) x (a -> (a ( b))) -- first try
//! D0+ f = (f , D f )
//! As desired, this altered specification is compositional:
//! D0+ (g ◦ f )
//! = (g ◦ f , D (g ◦ f ))
//! -- definition of D0+
//! = (g ◦ f , \lambdaa -> D g (f a) ◦ D f a) -- Theorem 1
//! Note that D0+ (g ◦ f ) is assembled entirely from components of D0+ g and D0+ f , which is to say from g, D g, f ,
//! and D f . Writing out g ◦ f as \lambdaa -> g (f a) underscores that the two parts of D0+ (g ◦ f ) a both involve f a.
...
//! (x) :: (a -> c) -> (b -> d ) -> (a x b -> c x d )
//! f x g = \lambda(a, b) -> (f a, g b)
...
//! 3 The precedence of "x" is tighter than that of "->" and "(", so a -> b x (a ( b) is equivalent to a -> (b x (a ( b)).
...
//! D (f x g) (a, b) = D f a x D g b
//! If f :: a -> c and g :: b -> d , then D f a :: a ( c and D g b :: b ( d , so both sides of this equation have type
//! a x b ( c x d.
//! Theorem 2 gives us what we need to construct D+ (f x g) compositionally:
//! Corollary 2.1 (Proved in Appendix C.2) D+ is compositional with respect to (x). Specifically,
//! D+ (f x g) (a, b) = let {(c, f 0 ) = D+ f a; (d , g 0 ) = D+ g b } in ((c, d ), f 0 x g 0 )
...
//! For example, consider the function id = \lambdaa -> a. Theorem 3 says that D id a = id . When expressed via
...
//! m x (m + n) (i.e., m rows and m + n columns) and is formed by the horizontal juxtaposition of an m x m
//! identity matrix on the left with an m x n zero matrix on the right. This m x (m + n) matrix, however, represents
//! fst :: Rm x Rn ( Rm . Note how much simpler it is to say D fst (a, b) = fst, and with no loss of precision!
//! Given Theorem 3, we can construct D+ f for all linear f :
//! Corollary 3.1 For all linear functions f , D+ f = \lambdaa -> (f a, f ). (Proof: immediate from the D+ definition and
//! Theorem 3.)
//!
//! 4
//!
...
//! build up differentiable functions compositionally, using exactly the forms introduced above, (namely (◦), (x)
...
//! new category is simply the representation that D+ produces: a -> b x (a ( b), considered as having domain a
...
//! 7
//!
//! Our goal is to discover a Category instance for D such that D is a functor. This goal is essentially an algebra
//! problem, and the desired Category instance is a solution to that problem. Saying that D is a functor is equivalent
//! to the following two conditions for all suitably typed functions f and g:8
//! id = D id
//! D g ◦ D f = D (g ◦ f )
//! Equivalently, by the definition of D,
//! id = D (D+ id )
//! D (D+ g) ◦ D (D+ f ) = D (D+ (g ◦ f ))
//! Now recall the following results from Corollaries 3.1 and 1.1:
//! D+ id = \lambdaa -> (id a, id )
//! D+ (g ◦ f ) = \lambdaa -> let {(b, f 0 ) = D+ f a; (c, g 0 ) = D+ g b } in (c, g 0 ◦ f 0 )
//! Then use these two facts to rewrite the right-hand sides of the functor specification for D:
//! id = D (\lambdaa -> (a, id ))
//! D (D+ g) ◦ D (D+ f ) = D (\lambdaa -> let {(b, f 0 ) = D+ f a; (c, g 0 ) = D+ g b } in (c, g 0 ◦ f 0 ))
//! The id equation is trivially solvable by defining id = D (\lambdaa -> (a, id )). To solve the (◦) equation, generalize it
//! to a stronger condition:9
//! D g ◦ D f = D (\lambdaa -> let {(b, f 0 ) = f a; (c, g 0 ) = g b } in (c, g 0 ◦ f 0 ))
//! The solution of this stronger condition is immediate, leading to the following instance as a sufficient condition
//! for D being a functor:
//! linearD :: (a -> b) -> D a b
//! linearD f = D (\lambdaa -> (f a, f ))
...
//! id ◦ D f
//! = D (\lambdab -> (b, id )) ◦ D f
//! = D (\lambdaa -> let {(b, f 0 ) = f a; (c, g 0 ) = (b, id )} in (c, g 0 ◦ f 0 ))
//! = D (\lambdaa -> let {(b, f 0 ) = f a } in (b, id ◦ f 0 ))
//! = D (\lambdaa -> let {(b, f 0 ) = f a } in (b, f 0 ))
//! = D (\lambdaa -> f a)
//! =D f
//!
...
//! 9 The new f
//!
//! is the old D+ f and so has changed type from a -> b to a -> b x (a ( b). Likewise for g.
...
//! only morphisms in D that arise from D, i.e., only fˆ :: D a b such that fˆ = D f for some f :: a -> b. We can
//! ensure that indeed only such fˆ do arise by making D a b an abstract type, i.e., hiding its data constructor . The
//! slightly more specialized requirement of our first identity property is then id ◦ D f = D f for any f :: a -> b,
...
//! = D f
//!
//! -- functor law for id (specification of D)
//! -- functor law for (◦)
//! -- category law
//!
...
//! D beyond the functor laws. The importance of this observation is that we never need to perform these proofs
...
//! specification that D is a functor, Corollary 2.1 leads to a correct Monoidal instance from the specification that
//! D is a monoidal functor, as we'll now see.
//! Let F be D in the reversed form of the monoidal functor equation above, and expand D to its definition as
//! D ◦ D+:
//! D (D+ f ) x D (D+ g) = D (D+ (f x g))
//! By Corollary 2.1,
//! D+ (f x g) = \lambda(a, b) -> let {(c, f 0 ) = D+ f a; (d , g 0 ) = D+ g b } in ((c, d ), f 0 x g 0 )
//! Now substitute the left-hand side of this equation into the right-hand side of the of the monoidal functor property
//! for D, and strengthen the condition by generalizing from D+ f and D+ g:
//!
//!
//! --- PAGE BREAK ---
//! The Simple Essence of Automatic Differentiation
...
//! 9
//!
//! D f x D g = D (\lambda(a, b) -> let {(c, f 0 ) = f a; (d , g 0 ) = g b } in ((c, d ), f 0 x g 0 ))
...
//! and (x)), also preserves the cartesian structure:
//! F exl = exl
//! F exr = exr
//! F dup = dup
//! Just as Corollaries 1.1 through 3.1 were key to deriving a correct-by-construction Category and Monoidal
//! instances from the specification that D is a functor and a monoidal functor respectively, Corollary 3.1 enables a
//! correct-by-construction Cartesian instance from the specification that D is a cartesian functor. Let F be D in
//! the reversed forms of cartesian functor equations above, and expand D to its definition as D ◦ D+:
//! exl = D (D+ exl )
//! exr = D (D+ exr )
//! dup = D (D+ dup)
//! Next, by Corollary 3.1, together with the linearity of exl , exr , and dup,
//! D+ exl = \lambdap -> (exl p, exl )
//! D+ exr = \lambdap -> (exr p, exr )
//! D+ dup = \lambdaa -> (dup a, dup)
//! Now substitute the left-hand sides of these three properties into the right-hand sides of the of the cartesian
//! functor properties for D, and recall the definition of linearD:
//! exl = linearD exl
//! exr = linearD exr
//! dup = linearD dup
...
//!
//! inlF :: Additive b => a -> a x b
//! inrF :: Additive a => b -> a x b
//! jamF :: Additive a => a x a -> a
//!
//! instance Cartesian (->+) where
//! exl = AddFun exl
//! exr = AddFun exr
...
//! Derived Operations
//!
//! With dup, we can define an alternative to (x) that takes two morphisms sharing a domain:
//! (M) :: Cartesian k => (a 'k ' c) -> (a 'k ' d ) -> (a 'k ' (c x d ))
//! f M g = (f x g) ◦ dup
//! The (M) operation is particularly useful for translating the \lambda-calculus to categorical form [Elliott, 2017, Section
//! 3].
//! Dually, jam lets us define a second alternative to (x) for two morphisms sharing a codomain:
//!
//!
//! --- PAGE BREAK ---
//! The Simple Essence of Automatic Differentiation
...
//! 11
//!
//! (O) :: Cocartesian k => (c 'k ' a) -> (d 'k ' a) -> ((c x d ) 'k ' a)
//! f O g = jam ◦ (f x g)
//! The (M) and (O) operations are invertible in uncurried form [Gibbons, 2002]:
//! fork :: Cartesian
//! unfork :: Cartesian
//!
//! k => (a 'k ' c) x (a 'k ' d ) -> (a 'k ' (c x d ))
//! k => (a 'k ' (c x d )) -> (a 'k ' c) x (a 'k ' d )
//!
//! join :: Cocartesian k => (c 'k ' a) x (d 'k ' a) -> ((c x d ) 'k ' a)
//! unjoin :: Cocartesian k => ((c x d ) 'k ' a) -> (c 'k ' a) x (d 'k ' a)
//! where
//! fork (f , g) = f M g
...
//! Numeric Operations
//!
//! So far, the vocabulary we've considered comprises linear functions and combining forms ((◦) and (x)) that
...
//! D mulC (a, b) = \lambda(da, db) -> da * b + a * db
//! Note the linearity of the right-hand side, so that the derivative of mulC at (a, b) for real values has the expected
//! type: R x R ( R.12 To make the linearity more apparent, and to prepare for variations later in this paper,
...
//! instance for differentiable functions by saying that D preserves (NumCat) structure, i.e., D negateC = negateC ,
//! D addC = addC , and D mulC = mulC . Reasoning as before, we get another correct-by-construction instance
//! for differentiable functions:
//! instance NumCat D where
//! negateC = linearD negateC
//! addC = linearD addC
//! mulC = D (\lambda(a, b) -> (a * b, scale b O scale a))
//! Similar reasoning applies to other numeric operations, e.g.,
//! instance FloatingCat D where
//! sinC = D (\lambdaa -> (sin a, scale (cos a)))
//! cosC = D (\lambdaa -> (cos a, scale (-sin a)))
//! expC = D (\lambdaa -> let e = exp a in (e, scale e))
//! ...
...
//! sqr = mulC ◦ (id M id )
//! 12 The derivative of uncurried multiplication generalizes to an arbitrary bilinear function f :: a x b -> c [Spivak, 1965, Problem
//! 2-12]: D f (a, b) = \lambda(da, db) -> f (da, b) + f (a, db).
//!
//!
//! --- PAGE BREAK ---
//! The Simple Essence of Automatic Differentiation
...
//! scale :: a -> (a 'k ' a)
//! (O) :: (a 'k ' c) -> (b 'k ' c) -> ((a x b) 'k ' c)
//! (M) :: (a 'k ' c) -> (a 'k ' d ) -> (a 'k ' (c x d ))
...
//! The Simple Essence of Automatic Differentiation
//!
//! newtype Dk a b = D (a -> b x (a 'k ' b))
//! linearD :: (a -> b) -> (a 'k ' b) -> Dk a b
//! linearD f f 0 = D (\lambdaa -> (f a, f 0 ))
...
//! addC
//! mulC
//! = D (\lambda(a, b) -> (a * b, scale b O scale a))
...
//! the rows of the 3 x 2 Jacobian matrix that represents the derivative of f at any given point p :: R2 .
...
//! and the identity functor (containing one element) [Magalhaes et al., 2010; Magalhaes et al., 2011]. One must
...
//! 13 Following Haskell notation for right sections, "(◦ f )" is shorthand for \lambdah -> h ◦ f .
//!
//!
//! --- PAGE BREAK ---
//! 18
...
//! instance (HasDot s a, HasDot s b) => HasDot s (a x b) where dot (u, v ) = dot u O dot v
...
//! Recall from Section 8, that scale forms 1 x 1 matrices, while (O) and (M) correspond to horizontal and vertical
//! juxtaposition, respectively. Thus, from a matrix perspective, duality is transposition, turning an m x n matrix
//! into an n x m matrix. Note, however, that Dual k involves no actual matrix computations unless k does. In
...
//!
//! Figure 12: \lambda((x , y), z ) -> cos (x + y * z ) in DDual (->+)
//!
//!
//! --- PAGE BREAK ---
//! The Simple Essence of Automatic Differentiation
...
//! replacing the two arguments to (x) by a (representable) functor h of morphisms:
...
//! unD :: D a b -> (a -> (b x (a ( b)))
//! unD (D f ) = f
//! unzip :: Functor h => h (a x b) -> h a x h b
//! unzip = fmap exl M fmap exr
//! second :: Monoidal k => (b 'k ' d ) -> ((a x b) 'k ' (a x d ))
//! second g = id x g
//! inIF :: (Additive a, Foldable h) => h (a -> h a)
//! inIF = tabulate (\lambdai a -> tabulate (\lambdaj -> if i = j then a else 0))
...
//! Theorem 6 (Proved in Appendix C.6) Given the definitions in Figure 13, D is a homomorphism with respect
//! to each instantiated class.
...
//! instance Terminal (->) where it = \lambda -> ()
//! instance Initial (->)
//! where ti = \lambda() -> 0
...
//! 0 = ti ◦ it
//! f + g = jam ◦ (f x g) ◦ dup -- = jam ◦ (f M g) = (f O g) ◦ dup.
...
//!
//! D+ (f x g) (a, b)
//! = ((f x g) (a, b), D (f x g) (a, b))
//! = ((f a, g b), D (f x g) (a, b))
//! = ((f a, g b), D f a x D g b)
//! = let {(c, f 0 ) = (f a, D f a); (d , g 0 ) = (g b, D g b)} in ((c, d ), f 0 x g 0 )
//! = let {(c, f 0 ) = D+ f a; (d , g 0 ) = D+ g b } in ((c, d ), f 0 x g 0 )
//!
//! C.3
//!
//! 27
...
//! cont (f x g)
//! = Cont (◦ (f x g))
//! = Cont (\lambdah -> h ◦ (f x g))
//! = Cont (\lambdah -> join (unjoin h) ◦ (f x g))
//! = Cont (\lambdah -> let (ha , hb ) = unjoin h in join (ha , hb ) ◦ (f x g))
//! = Cont (\lambdah -> let ... in (ha O hb ) ◦ (f x g))
//! = Cont (\lambdah -> let ... in (ha ◦ f O hb ◦ g))
//! = Cont (\lambdah -> let ... in ((◦ f ) ha O (◦ g) hb ))
//! = Cont (\lambdah -> let ... in join ((◦ f ) ha , (◦ g) hb ))
//! = Cont (\lambdah -> let ... in join (((◦ f ) x (◦ g)) (ha , hb )))
//! = Cont (\lambdah -> join (((◦ f ) x (◦ g)) (unjoin h)))
//! = Cont (join ◦ ((◦ f ) x (◦ g)) ◦ unjoin)
//!
//! -- definition of cont
//! -- definition of right section
//! -- join ◦ unjoin = id
...
//! -- definition of (◦)
//!
//! The crucial trick here was to note that h :: (a x b) 'k ' r can be split into two continuations ha :: a 'k ' r and
...
//! = Cont (\lambdah -> h ◦ exl )
//! = Cont (\lambdah -> h O 0)
//! = Cont (\lambdah -> join (h, 0))
//! = Cont (\lambdah -> join (inl h))
//! = Cont (join ◦ inl )
//!
//! -- definition of cont
//! -- Appendix B
...
//!
//! Similarly, cont exr = Cont (join ◦ inr ). For dup :: a 'k ' (a x a), we'll have h :: (a x a) ; r , so we can split h
//! with unjoin:
//! cont dup
//! = Cont (\lambdah -> h ◦ dup)
//! = Cont (\lambdah -> join (unjoin h) ◦ dup)
//! = Cont (\lambdah -> let (ha , hb ) = unjoin h in (ha O hb ) ◦ dup)
//! = Cont (\lambdah -> let (ha , hb ) = unjoin h in ha + hb )
//! = Cont (\lambdah -> let (ha , hb ) = unjoin h in jam (ha , hb ))
//! = Cont (\lambdah -> jam (unjoin h))
//! = Cont (jam ◦ unjoin)
//!
//! -- definition of cont
//! -- join ◦ unjoin = id
...
//! = Cont (\lambdah -> h ◦ inl )
//! -- definition of inl
//! = Cont (\lambdah -> join (unjoin h) ◦ inl )
//! -- join ◦ unjoin = id
//! = Cont (\lambdah -> let (ha , hb ) = unjoin h in (ha O hb ) ◦ inl ) -- definition of join
//!
//!
//! --- PAGE BREAK ---
//! The Simple Essence of Automatic Differentiation
//!
//! = Cont (\lambdah -> let (ha , hb ) = unjoin h in ha )
//! = Cont (\lambdah -> exl (unjoin h))
//! = Cont (exl ◦ unjoin)
//!
//! 29
//!
...
//! cont jam
//! = Cont (\lambdah -> h ◦ jam)
//! = Cont (\lambdah -> h ◦ (id O id ))
//! = Cont (\lambdah -> h ◦ id O h ◦ id )
//! = Cont (\lambdah -> h O h)
//! = Cont (\lambdah -> join (h, h))
//! = Cont (join ◦ dup)
//!
//! -- definition of cont
//! -- a law for jam and (O)
...
//! cont (scale s)
//! = Cont (\lambdah -> h ◦ scale s)
//! = Cont (\lambdah -> scale s ◦ h)
//! = Cont (\lambdah -> scale s h)
//! = Cont (scale s)
//!
//! -- definition of cont
...
//! 1. dot is linear.
//! 2. dot -1 is linear.
//! 3. unjoin ◦ dot = dot x dot
//! 4. dot -1 ◦ join = dot -1 x dot -1
...
//! unjoin ◦ dot
//! = \lambda(u, v ) -> unjoin (dot (u, v ))
//! = \lambda(u, v ) -> (dot (u, v ) ◦ inl , dot (u, v ) ◦ inr )
//! = \lambda(u, v ) -> (\lambdax -> dot (u, v ) (inl x ), \lambday -> dot (u, v ) (inr y))
//! = \lambda(u, v ) -> (\lambdax -> dot (u, v ) (x , 0), \lambday -> dot (u, v ) (0, y))
//! = \lambda(u, v ) -> (\lambdax -> dot u x + dot v 0, \lambday -> dot u 0 + dot v y)
//! = \lambda(u, v ) -> (\lambdax -> dot u x , \lambday -> dot v y)
//! = \lambda(u, v ) -> (dot u, dot v )
//! = dot x dot
//!
//! -- \eta-expansion
...
//! dot u O dot v
//! = jam ◦ (dot u x dot v )
//! = \lambda(x , y) -> jam ((dot u x dot v ) (x , y))
//! = \lambda(x , y) -> jam (dot u x , dot v y)
//! = \lambda(x , y) -> dot u x + dot v y
//! = \lambda(x , y) -> dot (u, v ) (x , y)
//! = dot (u, v )
//!
//! -- definition of (O)
//! -- definition of (◦) for functions
//! -- definition of (x) for functions
//! -- definition of jam for functions
...
//! For Monoidal , the homomorphism condition is asDual (f x g) = asDual f x asDual g. Simplify both sides:
//! asDual (Cont f ) x asDual (Cont g)
//! = Dual (onDot f ) x Dual (onDot g)
//!
//! -- definition of asDual
//!
//! asDual (Cont f x Cont g)
//! = asDual (Cont (join ◦ (f x g) ◦ unjoin))
//! = Dual (onDot (join ◦ (f x g) ◦ unjoin))
//! = Dual (dot -1 ◦ join ◦ (f x g) ◦ unjoin ◦ dot)
//! = Dual ((dot -1 x dot -1 ) ◦ (f x g) ◦ (dot x dot))
//! = Dual (dot -1 ◦ f ◦ dot x dot -1 ◦ g ◦ dot -1 )
//! = Dual (onDot f x onDot g)
//!
//! -- definition of (x) on Cont
//! -- definition of asDual
//! -- definition of onDot
//! -- Lemma 7, parts 3 & 4
//! -- law about (x)/(◦)
...
//! = Dual (\lambdau -> dot -1 (join (inl (dot u))))
//! = Dual (\lambdau -> dot -1 (join (dot u, 0)))
//! = Dual (\lambdau -> dot -1 (dot u O 0))
//! = Dual (\lambdau -> dot -1 (dot u O dot 0))
//! = Dual (\lambdau -> dot -1 (dot (u, 0)))
//! = Dual (\lambdau -> (u, 0))
//! = Dual (\lambdau -> inl u)
//! = Dual inl
//!
...
//! = Dual (\lambda(u, v ) -> dot -1 (jam (unjoin (dot (u, v )))))
//! = Dual (\lambda(u, v ) -> dot -1 (jam (dot u, dot v )))
//! = Dual (\lambda(u, v ) -> dot -1 (dot u + dot v ))
//! = Dual (\lambda(u, v ) -> dot -1 (dot u) + dot -1 (dot v ))
//! = Dual (\lambda(u, v ) -> u + v )
//! = Dual jam
//!
//! -- definition of asDual
...
//! = Dual (\lambda(u, v ) -> dot -1 (exl (unjoin (dot (u, v )))))
//! = Dual (\lambda(u, v ) -> dot -1 (exl (dot u, dot v )))
//! = Dual (\lambda(u, v ) -> dot -1 (dot u))
//! = Dual (\lambda(u, v ) -> u)
//! = Dual exl
//!
//! -- specification
...
//! = Dual (\lambdau -> dot -1 (join (dup (dot u))))
//! = Dual (\lambdau -> dot -1 (join (dot u, dot u)))
//! = Dual (\lambdau -> dot -1 (dot u O dot u))
//! = Dual (\lambdau -> dot -1 (dot (u, u)))
//! = Dual (\lambdau -> (u, u))
//! = Dual (\lambdau -> dup u)
//! = Dual dup
//!
...
//! D (f x g) (a, b) = D f a x D g b
//! Letting cross = uncurry (x), we can rephrase this theorem:
//! D (f x g)
//! = \lambda(a, b) -> D f a x D g b
//! = \lambda(a, b) -> cross (D f a, D g b)
//! = \lambda(a, b) -> cross ((D f x D g) (a, b))
//! = cross ◦ (D f x D g)
//!
//! -- Theorem 2
//! -- definition of cross
//! -- definition of (x) on functions
//! -- definition of (◦) on functions
//!
...
//! (f x f 0 ) M (g x g 0 ) = transpose ◦ ((f M g) x (g 0 M g 0 ))
//!
//!
//! --- PAGE BREAK ---
//! 34
...
//!
//! where
//! transpose :: ((a x b) x (c x d )) -> ((a x c) x (b x d ))
...
//! inl ◦ f M inl ◦ g
//! = \lambdaa -> (inl ◦ f M inl ◦ g) a
//! = \lambdaa -> (inl (f a), inl (g a))
//! = \lambdaa -> ((f a, 0), (g a, 0))
//! = \lambdaa -> transpose ((f a, g a), (0, 0))
//! = \lambdaa -> transpose ((f a, g a), 0)
//! = \lambdaa -> transpose (inl (f a, g a))
//! = transpose ◦ inl ◦ (f M g)
//!
//! -- \eta-expand
...
//! Francois Chollet. Keras resources. GitHub repository, 2016. URL https://github.com/fchollet.
...
//! Brendan Fong, David I. Spivak, and Remy Tuyeras. Backprop as functor: A compositional perspective on
...
//! Joachim Lambek. From \lambda-calculus to cartesian closed categories. In J.P. Seldin and J.R. Hindley, editors, To
...
//! York, 1998.
//! Hugo Daniel Macedo and Jose Nuno Oliveira. Typing linear algebra: A biproduct-oriented approach. Science of
//! Computer Programming, 78(11):2160-2191, 2013.
//! Jose Pedro Magalhaes, Atze Dijkstra, Johan Jeuring, and Andres Loh. A generic deriving mechanism for Haskell.
//! In Haskell Symposium, pages 37-48, 2010.
//! Jose Pedro Magalhaes et al. GHC.Generics, 2011. URL https://wiki.haskell.org/GHC.Generics. Haskell
//! wiki.
...
//! Jose Nuno Oliveira. Program Design by Calculation. Draft of textbook in preparation, 2018.
