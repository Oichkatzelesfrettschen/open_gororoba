...
/// Base URL for SPDF Cassini cruise merged data. Used by `cassini_fetch`.
pub const CASSINI_CRUISE_BASE: &str = "https://spdf.gsfc.nasa.gov/pub/data/cassini/merged/";
...
mod tests {
    ...
    fn test_cassini_to_omni_rtn_conversion() {
        ...
        assert!(
            (omni[0].bx_gse - (-0.066987)).abs() < 1e-4,
            "RTN rotated Bx"
        );
