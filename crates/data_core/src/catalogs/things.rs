...
use crate::{catalogs::hi_cube::HiCubeMetadata, fetcher::FetchError};
use std::{
...
#[cfg_attr(not(feature = "fetch"), allow(dead_code))]
pub(crate) fn things_name_key(raw: &str) -> String {
    raw.chars()
...
#[cfg_attr(not(feature = "fetch"), allow(dead_code))]
pub(crate) fn parse_manifest_entry_from_filename(
    filename: &str,
