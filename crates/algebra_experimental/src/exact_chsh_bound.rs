...
pub fn compute_exact_cd_chsh_bound(dim: usize, resolution: usize) -> f64 {
    if dim < 16 {
        return 0.0;
    } // No zero divisors in < 16D, no entanglement channel
    ...
    let (i_a, j_a, k_b, l_b, p_x) =
        valid_zd.expect("Must find a ZD with non-zero associator torque");
            ...
            for a_prime_idx in 0..resolution {
                for b_idx in 0..resolution {
                    for b_prime_idx in 0..resolution {
                        let s = e_matrix[a_idx][b_idx] - e_matrix[a_idx][b_prime_idx]
...
mod tests {
    ...
    fn test_sedenion_chsh_bound() {
        ...
        assert!(
            s_max <= 2.0 + 1e-4,
            "Algebra violated classical Bell Bound! S = {}",
            s_max
        );
        println!(
            "<EMOJI+2705> 16D CHSH Bound strictly <= 2.0. Cayley-Dickson Algebra is classically local!"
        );
    }
    ...
    fn test_chingon_chsh_bound() {
        ...
        println!(
            "<EMOJI+2705> 64D CHSH Bound strictly <= 2.0. No dimensional quantum anomaly detected."
        );
    }
}
