...
pub fn format_commutator_table(generators: &[Mat5; 24]) -> String {
    ...
    let pairs = [
        ("SU(3)", "SU(3)"), ("SU(2)", "SU(2)"), ("U(1)", "U(1)"),
        ("U(1)", "SU(3)"), ("U(1)", "SU(2)"), ("SU(3)", "SU(2)"),
        ("U(1)", "LQ"), ("SU(3)", "LQ"), ("SU(2)", "LQ"), ("LQ", "LQ"),
    ];

    for &(s1, s2) in &pairs {
        let idx1 = sector_ranges.iter().find(|s| s.0 == s1).unwrap().1.clone();
        let idx2 = sector_ranges.iter().find(|s| s.0 == s2).unwrap().1.clone();
