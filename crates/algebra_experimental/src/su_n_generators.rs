...
pub fn construct_annihilation_operators(
    basis: &[Sedenion; 16],
...
pub fn construct_su5_generators_algebraic(
    basis: &[Sedenion; 16],
    complex_structure: &Sedenion,
) -> Vec<QuantumState> {
    let a_ops = construct_annihilation_operators(basis, complex_structure);
    let ad_ops: Vec<QuantumState> = a_ops
        .iter()
        .map(|a| match a {
    ...
    let h2 = (ad_ops[0] * a_ops[0] + ad_ops[1] * a_ops[1] - ad_ops[2] * a_ops[2] * 2.0)
        * (1.0 / 12.0_f64.sqrt());
    let h3 = (ad_ops[0] * a_ops[0] + ad_ops[1] * a_ops[1] + ad_ops[2] * a_ops[2]
        - ad_ops[3] * a_ops[3] * 3.0)
        * (1.0 / 24.0_f64.sqrt());
    let h4 =
        (ad_ops[0] * a_ops[0] + ad_ops[1] * a_ops[1] + ad_ops[2] * a_ops[2] + ad_ops[3] * a_ops[3]
            - ad_ops[4] * a_ops[4] * 4.0)
            * (1.0 / 40.0_f64.sqrt());
...
mod tests {
    ...
    fn test_algebraic_su5_generator_construction() {
        ...
        assert_eq!(
            null_count, 0,
            "all SU(5) generators should be observable for standard basis"
        );
    }
}
