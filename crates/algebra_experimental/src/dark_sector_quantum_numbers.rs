...
pub fn extract_dark_quantum_numbers() -> Vec<[f64; 4]> {
    ...
    for (i, generator) in su5_gens.iter().enumerate() {
        if *generator != QuantumState::TopologicalNull
            && classify_generator(i) == GeneratorType::Dark
        {
    ...
    for dark_generator in dark_gens {
        ...
        for (i, cartan) in sm_cartans.iter().enumerate() {
            ...
            if let (QuantumState::Observable(s), QuantumState::Observable(d)) =
                (commutator, dark_generator)
            {
...
mod tests {
    ...
    fn test_dark_sector_sterility() {
        ...
        let all_sterile = charges
            .iter()
            .all(|c| c.iter().all(|&val| val.abs() < 1e-9));
        assert!(all_sterile);
    }
}
