...
fn test_delta_cp_from_gram_quartet() {
    ...
    let delta_quartet =
        gram_phases[0][0] + gram_phases[1][2] - gram_phases[0][2] - gram_phases[1][0];
    println!("\n  Rephasing-invariant quartet:");
    println!("  delta = phi[0][0] + phi[1][2] - phi[0][2] - phi[1][0]");
    println!(
        "        = {:.2} + {:.2} - {:.2} - {:.2}",
        gram_phases[0][0].to_degrees(),
        gram_phases[1][2].to_degrees(),
        gram_phases[0][2].to_degrees(),
        gram_phases[1][0].to_degrees()
    );
    ...
    println!(
        "\n  Complex quartet: {:.4} + {:.4}i",
        quartet.re, quartet.im
    );
...
fn test_delta_cp_all_flavor_assignments() {
    ...
    for &(e, mu, _tau, label) in &perms {
        ...
        println!("  {:>35} | {:>+10.2} | {:>+10.2}", label, delta, residual);
    }
    ...
    for &(e, mu, _tau, row_label) in &perms {
        for &(c1, c2, c3) in &col_perms {
            let cols = [c1, c2, c3];
            let q =
                gc[e][cols[0]] * gc[mu][cols[2]] * gc[e][cols[2]].conj() * gc[mu][cols[0]].conj();
            let delta = q.arg().to_degrees();
            let residual = ((delta - pdg_delta + 540.0) % 360.0) - 180.0;
...
fn test_delta_cp_full_bilateral() {
    ...
    let build_gram =
        |profiles_a: &[[f64; 16]], profiles_b: &[[f64; 16]]| -> [[Complex<f64>; 3]; 3] {
    ...
    let quartet =
        |g: &[[Complex<f64>; 3]; 3], e: usize, mu: usize, c1: usize, c3: usize| -> Complex<f64> {
    ...
    println!(
        "  Intra-charged quartet:  arg = {:.2} deg, |Q| = {:.4}",
        q_ch.arg().to_degrees(),
        q_ch.norm()
    );
    println!(
        "  Intra-neutrino quartet: arg = {:.2} deg, |Q| = {:.4}",
        q_nu.arg().to_degrees(),
        q_nu.norm()
    );
    println!(
        "  Cross-sector quartet:   arg = {:.2} deg, |Q| = {:.4}",
        q_cross.arg().to_degrees(),
        q_cross.norm()
    );
    ...
    let col_perms: [(usize, usize); 6] = [(0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1)];
    ...
    println!(
        "  Best: {} -> delta_CP = {:.2} deg (residual {:.2})",
    ...
    let results: Vec<_> = pairs
        ...
        .flat_map_iter(|&ch| {
            ...
            pairs.iter().map(move |&nu| {
                ...
                let ch_profiles: Vec<[f64; 16]> =
                    subs.iter().map(|s| build_profile(ch, s)).collect();
                let nu_profiles: Vec<[f64; 16]> =
                    subs.iter().map(|s| build_profile(nu, s)).collect();
...
fn test_cp_optimal_pair_with_psi() {
    ...
    let cp_pairs: [(usize, usize, usize, usize); 3] =
        [(11, 13, 11, 14), (9, 14, 9, 15), (10, 15, 10, 13)];
    ...
    for &(ch_a, ch_b, nu_a, nu_b) in &cp_pairs {
        ...
        let results: Vec<_> = grid
            .par_iter()
            .map(|&(a_ch, a_nu)| {
                let (m_ch, m_nu) = construct_pmns_matrices_two_param(ch_pair, nu_pair, a_ch, a_nu);
        ...
        println!(
            "  t12={:.2}, t13={:.2}, t23={:.2}, chi2={:.1}",
            best.3, best.4, best.5, best.0
        );
        println!(
            "  PDG errors: t12={:.1}%, t13={:.1}%, t23={:.1}%",
...
fn test_composite_selector_blend() {
    ...
    let angle_ch_profiles: Vec<[f64; 16]> =
        subs.iter().map(|s| build_profile(angle_ch, s)).collect();
    let angle_nu_profiles: Vec<[f64; 16]> =
        subs.iter().map(|s| build_profile(angle_nu, s)).collect();
    let cp_ch_profiles: Vec<[f64; 16]> = subs.iter().map(|s| build_profile(cp_ch, s)).collect();
    let cp_nu_profiles: Vec<[f64; 16]> = subs.iter().map(|s| build_profile(cp_nu, s)).collect();
    ...
    println!(
        "    ch: cos(angle,cp) = {:.4} (norms: {:.2}, {:.2})",
        overlap_ch / (norm_a_ch * norm_c_ch),
        norm_a_ch,
        norm_c_ch
    );
    ...
    println!(
        "    nu: cos(angle,cp) = {:.4} (norms: {:.2}, {:.2})",
        overlap_nu / (norm_a_nu * norm_c_nu),
        norm_a_nu,
        norm_c_nu
    );
    ...
    for w_step in 0..=20 {
        ...
        let blended_ch: Vec<[f64; 16]> = (0..3)
            .map(|i| blend(&angle_ch_profiles[i], &cp_ch_profiles[i]))
            .collect();
        let blended_nu: Vec<[f64; 16]> = (0..3)
            .map(|i| blend(&angle_nu_profiles[i], &cp_nu_profiles[i]))
            .collect();
...
fn test_split_angle_cp() {
    ...
    let angle_ch = (11_usize, 12);
    let angle_nu = (7_usize, 8);
    let (m_ch, m_nu) = construct_pmns_matrices_two_param(angle_ch, angle_nu, 3.75, 1.30);
    let eig_ch = m_ch.self_adjoint_eigen(faer::Side::Lower).unwrap();
    let eig_nu = m_nu.self_adjoint_eigen(faer::Side::Lower).unwrap();
    let u_raw = eig_ch.U().transpose() * eig_nu.U();
    ...
    for w_step in 0..=40 {
        ...
        for i in 0..3 {
            for j in 0..3 {
                let mag = u_real[(i, j)].abs();
                let sign = if u_real[(i, j)] >= 0.0 { 1.0 } else { -1.0 };
    ...
    println!(
        "  theta_12 = {:.2} deg (PDG: {:.2}, error: {:.2}%)",
        t12,
        pdg.theta_12_deg,
        ((t12 - pdg.theta_12_deg) / pdg.theta_12_deg * 100.0).abs()
    );
    println!(
        "  theta_13 = {:.2} deg (PDG: {:.2}, error: {:.2}%)",
        t13,
        pdg.theta_13_deg,
        ((t13 - pdg.theta_13_deg) / pdg.theta_13_deg * 100.0).abs()
    );
    println!(
        "  theta_23 = {:.2} deg (PDG: {:.2}, error: {:.2}%)",
        t23,
        pdg.theta_23_deg,
        ((t23 - pdg.theta_23_deg) / pdg.theta_23_deg * 100.0).abs()
    );
    println!("  |J_CP| = {:.4e} (PDG: ~3e-2)", best_j.abs());
    println!(
        "  delta_CP = {:.1} deg (PDG: -165, residual: {:.1} deg)",
        best_delta,
        best_resid.abs()
    );
    ...
    println!(
        "  chi2 (3 angles + delta_CP) = {:.2} / 4 dof = {:.2}",
        chi2_full,
        chi2_full / 4.0
    );
}
...
fn test_3blade_unified() {
    ...
    for &(ci, cj, ck, ni, nj, nk) in &candidates {
        ...
        let build_3blade_profile = |a: usize, b: usize, c: usize, sub: &[usize]| -> [f64; 16] {
            ...
            let build_pair_profile =
                |m1: &MajoranaMode, m2: &MajoranaMode, s: &[usize]| -> [f64; 16] {
        ...
        let ch_profiles: Vec<[f64; 16]> = subs
            .iter()
            .map(|s| build_3blade_profile(ci, cj, ck, s))
            .collect();
        let nu_profiles: Vec<[f64; 16]> = subs
            .iter()
            .map(|s| build_3blade_profile(ni, nj, nk, s))
            .collect();
        ...
        let best = alpha_grid
            .par_iter()
            .map(|&(a_ch, a_nu)| {
                let cb = construct_casimir_baseline(
                    crate::quark_sector::SubalgebraScheme::InterleavedStride,
                );
                let (m_base_ch, m_base_nu) = assemble_lepton_baseline(&cb);
                let mut m_ch = m_base_ch;
                let mut m_nu = m_base_nu;
...
fn test_two_selector_type_model() {
    use crate::{
        bell_inequality::rotate_sparse, lepton_mass_hierarchy::cd_braid_signed_friction,
        majorana_braiding::MajoranaMode,
    };
    use cd_kernel::gourlay_psi;
    ...
    let sel_ch_3blade: Vec<f64> = subs
        .iter()
        .map(|s| {
            let (a, b, c) = ch_triple;
            let ma = MajoranaMode {
    ...
    let sel_nu_3blade: Vec<f64> = subs
        .iter()
        .map(|s| {
            let (a, b, c) = nu_triple;
            let ma = MajoranaMode {
    ...
    let ch_profiles_2blade: Vec<[f64; 16]> =
        subs.iter().map(|s| build_profile(angle_ch, s)).collect();
    let nu_profiles_2blade: Vec<[f64; 16]> =
        subs.iter().map(|s| build_profile(angle_nu, s)).collect();
    ...
    println!(
        "    ch: [{:.2}, {:.2}, {:.2}]",
        sel_ch_3blade[0], sel_ch_3blade[1], sel_ch_3blade[2]
    );
    println!(
        "    nu: [{:.2}, {:.2}, {:.2}]",
        sel_nu_3blade[0], sel_nu_3blade[1], sel_nu_3blade[2]
    );
    ...
    for beta_step in 1..=20 {
        ...
        for ach_step in 0..=10 {
            for anu_step in 0..=10 {
                ...
                let eig_ch = m_ch_sym.self_adjoint_eigen(faer::Side::Lower).unwrap();
                let eig_nu = m_nu_sym.self_adjoint_eigen(faer::Side::Lower).unwrap();
                let u_raw = eig_ch.U().transpose() * eig_nu.U();
                let (u_pmns, _, _) = crate::quark_sector::extract_ckm_permutation_aware(&u_raw);
    ...
    println!(
        "  alpha_ch = {:.1}, alpha_nu = {:.1}",
        best_scaled.2, best_scaled.3
    );
    println!(
        "  theta_12 = {:.2} deg (PDG: {:.2}, err: {:.1}%)",
        best_scaled.4,
        pdg.theta_12_deg,
        ((best_scaled.4 - pdg.theta_12_deg) / pdg.theta_12_deg * 100.0).abs()
    );
    println!(
        "  theta_13 = {:.2} deg (PDG: {:.2}, err: {:.1}%)",
        best_scaled.5,
        pdg.theta_13_deg,
        ((best_scaled.5 - pdg.theta_13_deg) / pdg.theta_13_deg * 100.0).abs()
    );
    println!(
        "  theta_23 = {:.2} deg (PDG: {:.2}, err: {:.1}%)",
        best_scaled.6,
        pdg.theta_23_deg,
        ((best_scaled.6 - pdg.theta_23_deg) / pdg.theta_23_deg * 100.0).abs()
    );
    println!(
        "  r = {:.4} (PDG: {:.4}, err: {:.1}%)",
        best_scaled.7,
        pdg_r,
        ((best_scaled.7 - pdg_r) / pdg_r * 100.0).abs()
    );
