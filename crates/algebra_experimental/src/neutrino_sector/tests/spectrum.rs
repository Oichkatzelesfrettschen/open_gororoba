...
fn test_cp_violation_joint_3d_scan() {
    ...
    let inner_angles = |t_s: f64, t_a: f64| -> (f64, f64, f64) {
        ...
        for kk in 0..6 {
            beta[kk] = t_s * u_solar[kk] + t_a * u_atmo[kk];
        }
        angles_at(&beta)
    };
    ...
    let results: Vec<_> = (1..=7_usize).into_par_iter().filter_map(scan_k).collect();
    ...
    if let Some(&(k_best, alpha_c, ts_c, ta_c, t12_c, t13_c, t23_c, jcp_c, delta_c)) = winner {
        println!(
            "  Coarse best k={}: alpha_CP={:.4}, t_sol={:.3}, t_atm={:.3}",
            k_best, alpha_c, ts_c, ta_c
        );
        println!(
            "  Coarse |J_CP| = {:.4e}, delta = {:.1} deg",
            jcp_c.abs(),
            delta_c
        );
        ...
        for &(kk, a, ts2, ta2, _t12b, _t13b, _t23b, jcpb, db) in &results {
            println!(
                "    k={}: alpha={:.3}, t_sol={:.3}, t_atm={:.3}, |J|={:.3e}, delta={:.1}",
                kk,
                a,
                ts2,
                ta2,
        ...
        for a_step in -5..=5_i32 {
            ...
            for ts_step in -5..=5_i32 {
                ...
                for ta_step in -5..=5_i32 {
                    ...
                    let r = evaluate_cp_scan_point_cardano(
                        alpha_cp, t_sol_f, t_atm_f, &phi_fine, &ctx_fine,
                    );
                    ...
                    if r.j_cp.abs() > fine_best_jcp.abs() {
                        ...
                        fine_best = (
                            alpha_cp, t_sol_f, t_atm_f, r.theta_12, r.theta_13, r.theta_23,
                            r.delta_cp,
                        );
                    }
                }
        ...
        println!(
            "  Refined k={}: alpha_CP={:.4}, t_sol={:.4}, t_atm={:.4}",
            k_best, alpha, ts, ta
        );
        println!(
            "  Angles: theta_12={:.2}, theta_13={:.2}, theta_23={:.2}",
            t12, t13, t23
        );
        println!(
            "  |J_CP| = {:.4e}, delta_CP = {:.1} deg",
            fine_best_jcp.abs(),
            delta
        );
        ...
        for i in 0..3 {
            bufs_fine.m_nu[(i, i)] = faer::c64::new(m_nu_f[(i, i)], 0.0);
            bufs_fine.m_ch[(i, i)] = faer::c64::new(m_ch_real[(i, i)], 0.0);
            for j in (i + 1)..3 {
                let phase = alpha * phi_fine[i][j];
                let mag = m_nu_f[(i, j)];
                bufs_fine.m_nu[(i, j)] = faer::c64::new(mag * phase.cos(), mag * phase.sin());
                bufs_fine.m_nu[(j, i)] = faer::c64::new(mag * phase.cos(), -mag * phase.sin());
                bufs_fine.m_ch[(i, j)] = faer::c64::new(m_ch_real[(i, j)], 0.0);
                bufs_fine.m_ch[(j, i)] = faer::c64::new(m_ch_real[(j, i)], 0.0);
            }
        }
        let eig_ch_r = bufs_fine
            .m_ch
            .self_adjoint_eigen(faer::Side::Lower)
            .unwrap();
        let eig_nu_r = bufs_fine
            .m_nu
            .self_adjoint_eigen(faer::Side::Lower)
            .unwrap();
        let u_r = eig_ch_r.U().adjoint() * eig_nu_r.U();
        let u_at_r = |i: usize, j: usize| -> faer::c64 { u_r[(perm_u[i], perm_d[j])] };

        let jarlskog_q = u_at_r(0, 1) * u_at_r(1, 2) * u_at_r(0, 2).conj() * u_at_r(1, 1).conj();
        ...
        println!(
            "  atan2(sin,cos) invariant = {:.1} deg",
            r_final.delta_cp_invariant
        );
        println!("  PDG NuFIT 5.3: delta = 195 +/- 25 deg (NO)");

        // ----- Nelder-Mead refinement (B2) -----
        println!(
            "\n  --- Nelder-Mead refinement (k={}, constrained fit) ---",
            k_best
        );
        let (nm_result, nm_params) =
            refine_cp_nelder_mead(&ctx_fine, &phi_fine, alpha, ts, ta, false);
        println!(
            "  NM params: alpha={:.4}, t_sol={:.4}, t_atm={:.4}",
            nm_params[0], nm_params[1], nm_params[2]
        );
        println!(
            "  NM angles: t12={:.2}, t13={:.2}, t23={:.2}",
            nm_result.theta_12, nm_result.theta_13, nm_result.theta_23
        );
        println!(
            "  NM |J_CP| = {:.4e}, delta_inv = {:.1} deg",
            nm_result.j_cp.abs(),
            nm_result.delta_cp_invariant
        );

        // Prediction mode (B2): pure -|J| cost, no angle penalty
        let (nm_pred, nm_pred_params) =
            refine_cp_nelder_mead(&ctx_fine, &phi_fine, alpha, ts, ta, true);
        println!("\n  --- Nelder-Mead prediction mode ---");
        println!(
            "  Pred params: alpha={:.4}, t_sol={:.4}, t_atm={:.4}",
            nm_pred_params[0], nm_pred_params[1], nm_pred_params[2]
        );
        println!(
            "  Pred angles: t12={:.2}, t13={:.2}, t23={:.2}",
        ...
        for &w in &[0.01, 0.1, 1.0, 10.0] {
            let (nm_r, nm_r_params) =
                refine_cp_nelder_mead_r(&ctx_fine, &phi_fine, alpha, ts, ta, false, w);
            // Compute r at this point
            let mut beta_r = [0.0_f64; 6];
            for kk in 0..6 {
                beta_r[kk] =
                    nm_r_params[1] * ctx_fine.u_solar[kk] + nm_r_params[2] * ctx_fine.u_atmo[kk];
            }
            ...
            println!(
                "  w={w:5.2}: t12={:.2}({err_12:.1}%), t13={:.2}({err_13:.1}%), t23={:.2}({err_23:.1}%), |J|={:.3e}, r={r_val:.4}",
                nm_r.theta_12,
                nm_r.theta_13,
                nm_r.theta_23,
                nm_r.j_cp.abs()
        ...
        let angles_at_nm = |beta: &[f64; 6]| -> (f64, f64, f64) {
            ...
            for i in 0..3 {
                for j in 0..3 {
                    u_perm2[(i, j)] = u_raw[(perm_u[i], perm_d[j])];
                }
            }
        ...
        for mu in 0..n_basis {
            ...
            for kk in 0..6 {
                bp[kk] = nm_params[1] * ctx_fine.u_solar[kk] + nm_params[2] * ctx_fine.u_atmo[kk];
                bm[kk] = bp[kk];
            }
            bp[mu] += eps_g;
            bm[mu] -= eps_g;
        ...
        let dot6 =
            |a: &[f64; 6], b: &[f64; 6]| -> f64 { a.iter().zip(b).map(|(x, y)| x * y).sum() };
        ...
        for (name, g_old, g_new) in [
            ...
            if n_old > 1e-15 && n_new > 1e-15 {
                ...
                println!(
                    "  {name}: principal angle = {pa_deg:.1} deg, |g_old|={n_old:.3}, |g_new|={n_new:.3}"
                );
                if pa_deg > 10.0 {
                    println!(
                        "    ** WARNING: > 10 deg drift -- branch/gauge frame may have changed across parameter points **"
                    );
                }
            }
        }
        ...
        for kk in 0..6 {
            beta_nm[kk] = nm_params[1] * ctx_fine.u_solar[kk] + nm_params[2] * ctx_fine.u_atmo[kk];
        }
        ...
        for i in 0..3 {
            m_nu_c[(i, i)] = faer::c64::new(m_nu_nm[(i, i)], 0.0);
            for j in (i + 1)..3 {
                let phase = nm_params[0] * phi_fine[i][j];
                let mag = m_nu_nm[(i, j)];
                m_nu_c[(i, j)] = faer::c64::new(mag * phase.cos(), mag * phase.sin());
                m_nu_c[(j, i)] = faer::c64::new(mag * phase.cos(), -mag * phase.sin());
            }
        }
        let eig_nu_nm = m_nu_c.self_adjoint_eigen(faer::Side::Lower).unwrap();
        let mut ev_nu: Vec<f64> = (0..3)
            .map(|i| eig_nu_nm.S().column_vector()[i].re.abs())
            .collect();
        ...
        let r_mass = if dm31_sq.abs() > 1e-30 {
            dm21_sq / dm31_sq
        } else {
            0.0
        };
        ...
        println!(
            "  theta_12 (deg)   | {:7.2}    | {:7.2}    | {:+.2}",
            nm_result.theta_12, 33.41, pull_12
        );
        println!(
            "  theta_13 (deg)   | {:7.2}    | {:7.2}    | {:+.2}",
            nm_result.theta_13, 8.54, pull_13
        );
        println!(
            "  theta_23 (deg)   | {:7.2}    | {:7.2}    | {:+.2}",
        ...
        println!(
            "  r = dm21/dm31    | {:7.4}    | {:7.4}    | {:+.2}",
            r_mass, r_pdg, pull_r
        );
        let chi2_total = pull_12 * pull_12
            + pull_13 * pull_13
            + pull_23 * pull_23
            + pull_j * pull_j
            + pull_r * pull_r;
...
fn test_cardano_vs_faer_scan_point() {
    ...
    let test_points = [(0.1, 1.0, 2.0), (0.3, 0.5, 3.0), (0.45, 1.5, 4.0)];
    ...
    for &(alpha, ts, ta) in &test_points {
        ...
        println!(
            "  alpha={alpha:.2}, ts={ts:.1}, ta={ta:.1}: dt12={d12:.2e}, dt13={d13:.2e}, dt23={d23:.2e}, dJ={dj:.2e}"
        );

        assert!(
            d12 < 1e-6,
            "theta_12 mismatch: faer={:.6}, cardano={:.6}",
            r_faer.theta_12,
            r_card.theta_12
        );
        assert!(
            d13 < 1e-6,
            "theta_13 mismatch: faer={:.6}, cardano={:.6}",
            r_faer.theta_13,
            r_card.theta_13
        );
        assert!(
            d23 < 1e-6,
            "theta_23 mismatch: faer={:.6}, cardano={:.6}",
            r_faer.theta_23,
            r_card.theta_23
        );
        assert!(
            dj < 1e-8,
            "j_cp mismatch: faer={:.10}, cardano={:.10}",
            r_faer.j_cp,
            r_card.j_cp
        );
    }
...
fn test_pathion_vk_spectrum() {
    ...
    println!(
        "  dim=16: rank={}, assessors={}",
        basis_16.nrows(),
        assess_16.len()
    );
    println!(
        "  dim=32: rank={}, assessors={}",
        basis_32.nrows(),
        assess_32.len()
    );
    ...
    println!(
        "  dim=64: rank={}, assessors={}",
        basis_64.nrows(),
        assess_64.len()
    );
    println!(
        "  Pattern: 6 -> {} -> {} (rank drops with doubling).",
        basis_32.nrows(),
        basis_64.nrows()
    );
    if basis_64.nrows() == 0 {
        ...
        println!(
            "  NOTE: dim=64 rank={} may be noise -- check VK_PROFILE output.",
            basis_64.nrows()
        );
    }
}
    ...
    for &(sel_a, sel_b) in &selector_pairs {
        for &lr_swap in &lr_swaps {
            for &eps_flip in &eps_signs {
                ...
                let u_raw_0 = eig_ch_0.U().transpose() * eig_nu_0.U();
                let (_, perm_u, perm_d) =
                    crate::quark_sector::extract_ckm_permutation_aware(&u_raw_0);
                ...
                let inner = |t_s: f64, t_a: f64| -> (f64, f64, f64) {
                    let mut beta = [0.0_f64; 6];
                    for kk in 0..6 {
                        beta[kk] = t_s * u_s[kk] + t_a * u_a[kk];
                    }
                    angles_at(&beta)
                };
                let (ts, ta, _, _) =
                    gauss_newton_2d(&inner, 1.5, 0.0, (33.41, 8.54, 49.0), (1.0, 2.24, 1.0), 15);
...
fn test_s3_action_on_v6_and_lift_derivation() {
    use cd_kernel::{gourlay_epsilon, gourlay_psi};
    ...
    println!(
        "\n  Eigenvalues of psi on V_6: {:?}",
        eig_psi.eigenvalues.as_slice()
    );
    let eig_eps = m_eps_v6.clone().symmetric_eigen();
    println!(
        "  Eigenvalues of eps on V_6: {:?}",
        eig_eps.eigenvalues.as_slice()
    );
}
...
fn test_psi_friction_profiles_42x3() {
    use crate::{lepton_mass_hierarchy::cd_braid_signed_friction, majorana_braiding::MajoranaMode};
    let (o1, o2, o3, sign_table) = super::psi_setup();
    let subs = [&o1[..], &o2[..], &o3[..]];
    ...
    for (a_idx, &(low, high)) in assessors.iter().enumerate() {
        ...
        for (g, sub) in subs.iter().enumerate() {
            friction_matrix[a_idx][g] =
                cd_braid_signed_friction(&mode_i, &mode_j, sub, &sign_table);
        }
    ...
    for (a_idx, &(low, high)) in assessors.iter().enumerate() {
        ...
        println!(
            "  {:>3} ({:>2},{:>2}) {:>5} | {:>8.3} {:>8.3} {:>8.3} | {:>5}",
            a_idx, low, high, gen_labels[a_idx], f[0], f[1], f[2], dom
        );

        let entry = type_friction_sums
            .entry(gen_labels[a_idx].clone())
            .or_insert([0.0; 3]);
        for g in 0..3 {
            entry[g] += f[g].abs();
        }
...
fn test_faer_vs_nalgebra_eigendecomp() {
    ...
    assert!(
        ortho_err < 1e-10,
        "faer basis non-orthonormal: |B*B^T - I|_F = {:.3e}",
        ortho_err
    );
    ...
    assert!(
        ortho_err_nal < 1e-10,
        "nalgebra basis non-orthonormal: |B*B^T - I|_F = {:.3e}",
        ortho_err_nal
    );
    ...
    assert!(
        proj_diff < 1e-8,
        "projector disagreement: |P_faer - P_nal|_F = {:.3e}",
        proj_diff
    );
    ...
    println!(
        "    sv[0] = {:.8e} (faer) vs {:.8e} (nalgebra)",
        sv_faer[0], sv_nal[0]
    );
    println!(
        "    |B*B^T - I|_F: faer={:.3e}, nalgebra={:.3e}",
        ortho_err, ortho_err_nal
    );
