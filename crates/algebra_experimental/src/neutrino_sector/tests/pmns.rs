...
fn test_neutrino_mass_matrix_and_see_saw() {
    ...
    for i in 0..4 {
        for j in 0..4 {
            if let (QuantumState::Observable(g1), QuantumState::Observable(g2)) =
                (dark_gens[i], dark_gens[j])
            {
                let product = g1.conj() * g2;
                mass_matrix[(i, j)] = product.to_slice()[0];
            }
        }
    }
    ...
    let m_r = mass_matrix[(3, 3)];
    let m_d = mass_matrix.get(0..3, 3..4);
    let m_light = m_d * m_d.transpose() * (1.0 / m_r);

    let eig = m_light.self_adjoint_eigen(Side::Lower).unwrap();
...
fn test_pmns_selector_pair_scan() {
    ...
    if let Some(&(score, ch, nu, (t12, t13, t23))) = all_results.first() {
        ...
        if pmns.delta_m31_sq.abs() > 1e-20 {
            println!(
                "  ratio delta_m21^2 / delta_m31^2 = {:.4} (PDG ~ 0.030)",
    ...
    for (rank, (score, ch, nu, (t12, t13, t23))) in all_results.iter().take(5).enumerate() {
        println!(
            "  #{}: ch=(e_{},e_{}), nu=(e_{},e_{}) | t12={:.1}, t13={:.1}, t23={:.1} | score={:.4}",
            rank + 1,
            ch.0,
            ch.1,
            nu.0,
    ...
    if let Some(&(_, ch, nu, (t12, t13, t23))) = all_results.first() {
        println!(
            "  PMNS best: ch=(e_{},e_{}), nu=(e_{},e_{})",
            ch.0, ch.1, nu.0, nu.1
        );
        println!("  CKM  best: up=(e_11,e_12), down=(e_10,e_11)");
        println!(
            "  theta_12 ratio (PMNS/CKM): {:.2} (observed: {:.2})",
            t12 / 14.19,
            pdg_t12 / 12.99
        );
        println!(
            "  theta_23 ratio (PMNS/CKM): {:.2} (observed: {:.2})",
            t23 / 2.52,
            pdg_t23 / 2.40
        );
        println!(
            "  theta_13 ratio (PMNS/CKM): {:.2} (observed: {:.2})",
            t13 / 0.22,
            pdg_t13 / 0.214
        );
    }
...
fn test_electroweak_mixing_angle_from_associator_flux() {
    ...
    let correction =
        (5.0 / 8.0) * (b1 - b2) / (2.0 * std::f64::consts::PI) * ln_gut_mz / alpha_gut_inv;
    ...
    println!(
        "    sin^2(theta_W) one-loop = {:.6} (PDG: {:.6})",
...
fn test_pmns_theta23_alpha_scan() {
    ...
    println!(
        "  Selectors: ch=(e_{},e_{}), nu=(e_{},e_{})",
        ch_pair.0, ch_pair.1, nu_pair.0, nu_pair.1
    );
    println!(
        "  sel_ch = [{:.4}, {:.4}, {:.4}]",
        sel_ch[0], sel_ch[1], sel_ch[2]
    );
    println!(
        "  sel_nu = [{:.4}, {:.4}, {:.4}]",
        sel_nu[0], sel_nu[1], sel_nu[2]
    );
...
fn test_pmns_theta23_composite_scan() {
    ...
    let combos_reduced: Vec<(usize, usize)> = (0..n)
        .flat_map(|c| (0..n).filter(move |&d| d != c).map(move |d| (c, d)))
        .collect();

    println!(
        "Scanning {} nu composite pairs (ch fixed to (e_11,e_12))",
        combos_reduced.len()
    );

    let results: Vec<(f64, usize, usize, (f64, f64, f64))> = combos_reduced
        .par_iter()
        .map(|&(c_idx, d_idx)| {
    ...
    for (rank, (score, c, d, (t12, t13, t23))) in sorted.iter().take(5).enumerate() {
        println!(
            "  #{}: nu=avg(({},{}),({},{})) | t12={:.1}, t13={:.1}, t23={:.1} | score={:.4}",
            rank + 1,
            pairs[*c].0,
            pairs[*c].1,
            pairs[*d].0,
...
fn test_pmns_offdiag_full_profile() {
    ...
    println!(
        "  Score: {:.6} (sparse: 0.110, diagonal: 0.132)",
        best_score
    );
...
fn test_pmns_offdiag_two_param() {
    ...
    let grid: Vec<(f64, f64)> = (0..100)
        .flat_map(|i| (0..100).map(move |j| (i as f64 * 0.05, j as f64 * 0.05)))
        .collect();

    let results: Vec<(f64, f64, f64, (f64, f64, f64))> = grid
        .par_iter()
        .map(|&(a_ch, a_nu)| {
            let mut m_ch = m_base_ch.clone();
            let mut m_nu = m_base_nu.clone();
...
fn test_pmns_type_x_defect_coupling() {
    ...
    println!(
        "  Best triad: (e_{}, e_{}, e_{})",
        best_triad.0, best_triad.1, best_triad.2
    );
    println!(
        "  Defect norm: {:.4}",
        best_defect.iter().map(|x| x * x).sum::<f64>().sqrt()
    );
    ...
    let overlap: f64 = best_defect
        .iter()
        .zip(psi_d.iter())
        .map(|(a, b)| a * b)
        .sum();
    println!(
        "  Overlap/norm ratio = {:.4} (cos(120) = -0.5)",
        overlap / self_norm
    );
}
...
fn test_v6_solar_angle_extraction() {
    ...
    for k in 0..rank_v6.min(6) {
        ...
        println!(
            "    V_6[{}]: solar={:.3}, reactor={:.3}, atmo={:.3}, solar_frac={:.3}",
    ...
    if !solar_v6_weights.is_empty() {
        ...
        for step in -40..=40_i32 {
            ...
            let (m_ch, m_nu) = construct_pmns_matrices_offdiag(ch_pair, nu_pair, 3.75);

            // Add second psi coupling (nu-specific)
            let (_, m_nu2) = construct_pmns_matrices_offdiag(ch_pair, nu_pair, 1.30);
            ...
            for i in 0..3 {
                for j in 0..3 {
                    ...
                    let base = m_nu[(i, j)] + (m_nu2[(i, j)] - m_nu[(i, j)]) * (1.30 / 3.75);
                    m_nu_combined[(i, j)] = base;
                }
            }
        ...
        println!(
            "  theta_12 = {:.2} deg (PDG: 33.41, error: {:.1}%)",
            best_angles.0,
            ((best_angles.0 - 33.41) / 33.41 * 100.0).abs()
        );
        println!(
            "  theta_13 = {:.2} deg (PDG: 8.54, error: {:.1}%)",
            best_angles.1,
            ((best_angles.1 - 8.54) / 8.54 * 100.0).abs()
        );
        println!(
            "  theta_23 = {:.2} deg (PDG: 49.0, error: {:.1}%)",
            best_angles.2,
            ((best_angles.2 - 49.0) / 49.0 * 100.0).abs()
        );
...
fn test_pmns_two_param_baseline_regression() {
    ...
    assert!(
        ...
        "theta_13 regression: got {:.4}, expected ~8.63 (tol 0.1 deg)",
        theta_13
    );
    ...
    for col in 0..3 {
        ...
        assert!(
            dot_ch.abs() > 0.99,
            "Charged eigenvector {} overlap: {:.6} (expected > 0.99)",
            col,
            dot_ch.abs()
        );
        assert!(
            dot_nu.abs() > 0.99,
            "Neutrino eigenvector {} overlap: {:.6} (expected > 0.99)",
            col,
            dot_nu.abs()
        );
    }
...
fn test_pmns_casimir_isolation() {
    use crate::quark_sector::{CasimirBaseline, SubalgebraScheme, assemble_quark_matrices};
    ...
    for i in 0..3 {
        for j in 0..3 {
            cb_flipped.c_su2[(i, j)] = -cb.c_su2[(i, j)];
        }
    }
    ...
    for i in 0..3 {
        for j in 0..3 {
            quark_diff += (m_up_flipped[(i, j)] - m_up_std[(i, j)]).abs();
        }
    }
    assert!(
        quark_diff > 0.1,
        "Flipped quark convention should differ: diff = {:.6}",
        quark_diff
    );
    ...
    for i in 0..3 {
        for j in 0..3 {
            assert!(
                (m_ch_std[(i, j)] - m_ch_check[(i, j)]).abs() < 1e-14,
                "Lepton M_ch changed when quark convention changed at ({},{})",
                i,
                j
            );
            assert!(
                (m_nu_std[(i, j)] - m_nu_check[(i, j)]).abs() < 1e-14,
                "Lepton M_nu changed when quark convention changed at ({},{})",
                i,
                j
            );
        }
