...
pub fn execute_all_prototypes() {
    ...
    println!(
        "[1/25] ZD Routing Null Code Syndrome: {:.4} (Expected ~0.0)",
        syn
    );
    ...
    println!(
        "[2/25] Surreal Clustering: Items grouped into {} classes",
        labels.iter().max().unwrap_or(&0)
    );
    ...
    println!(
        "[4/25] Topological Defect Sensing: Detected anomalies at indices {:?}",
        anomalies
    );
    ...
    println!(
        "[5/25] Algebraic Sieve: Discovered {} hidden supersymmetric dualities",
        pairs.len()
    );
    ...
    println!(
        "[6/25] Homotopic Pruning: Pruned {} trivial associative links",
        pruned
    );
    ...
    println!(
        "[7/25] Sedenion Raytracer: Geodesic fractured? {}",
        frac.is_some()
    );
    ...
    println!(
        "[8/25] ZPE LBM Collision: State deviated to norm {:.8} (equilibrium was 0.50000000)",
        dev
    );
    ...
    println!(
        "[9/25] ZD Tensor Network: Contraction skipped? {}",
        cont.is_none()
    );
    ...
    println!(
        "[10/25] Fano Cloaking: Tuned metamaterial vector max value: {:.4}",
        mat.iter().fold(0.0, |a: f64, b: &f64| a.max(b.abs()))
    );
    ...
    println!(
        "[11/25] Braid Compiler: Compiled state real component: {:.4}",
        state[0]
    );
    ...
    let mut zd = [0.0; 16];
    zd.copy_from_slice(&zd_vec[..16]);
    let bucket = resolve_collision_zd(&key, &zd);
    println!(
        "[12/25] ZD Hash Map: Key projected to bucket magnitude: {:.4} (Expected 0.0)",
        cd_norm_sq(&bucket)
    );
    ...
    println!(
        "[13/25] Sedenion Kalman: Topological uncertainty: {:.4}",
        uncert
    );
    ...
    println!(
        "[14/25] p-Adic Crypto: Masked real component: {:.4}",
        cipher[0]
    );
    ...
    n_b[2] = 1.0;
    println!(
        "[15/25] Spin Emanation Routing: Imaginary Flux: {:.4}",
        route_spin_network(&n_a, &n_b)
    );
    ...
    let fbase = [0.5; 16];
    println!(
        "[16/25] Fractal Pathfinding: Traversal Cost: {:.4}",
        fractal_heuristic(&fa, &fb, &fbase)
    );
    ...
    ry[2] = 1.0;
    let mut rz = [0.0; 16];
    rz[4] = 1.0; // The triplet e1, e2, e4 is non-associative
    let rnd = generate_next_random(&rx, &ry, &rz);
    println!(
        "[17/25] NA-PRNG: Chaotic next state max deviation: {:.4}",
        rnd.iter().fold(0.0, |a: f64, b: &f64| a.max(b.abs()))
    );

    // 18. Sedenion SVM
    println!(
        "[18/25] Sedenion SVM: Margin Separation: {:.4} (Expected 0.0)",
        sedenion_zd_kernel(&key, &zd)
    );

    // 19. Void Compression
    let empty = [0.0; 16];
    println!(
        "[19/25] Topological Void Compression: Is void? {}",
        is_topological_void(&empty)
    );
    ...
    println!(
        "[20/25] Zhilina Graph DB: Node traversal reached real value: {:.4}",
        z_target[0]
    );
    ...
    for (i, e) in env.iter_mut().enumerate() {
        *e = (i as f64).exp().fract();
    } // Non-alternative "noisy" environment
    let surv = update_cell(&cell, &env);
    println!("[21/25] Hypercomplex CA: Cell survived? {}", surv[0] > 0.0);

    // 22. Mass Ladder Predictor
    println!(
        "[22/25] Cosmological Mass Predictor: 8D->16D drop from 511keV: {:.2} eV",
        predict_next_mass_scale(511_000.0, 8, 16)
    );
    ...
    println!(
        "[23/25] Associator Descent: Gradient step modified real weight to: {:.4}",
        next_w[0]
    );
    ...
    println!(
        "[24/25] Sedenion Dithering: Error diffused real value: {:.4}",
        diff[0]
    );
