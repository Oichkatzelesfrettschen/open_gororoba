...
pub fn zpe_bgk_collision(
    ...
    let scale = if zpe_energy > 0.0 { 1e-4 / zpe_energy.sqrt() } else { 0.0 };
...
mod tests {
    ...
    fn test_zpe_injection() {
        ...
        vacuum[1] = 1.0; vacuum[2] = 1.0; // e1 and e2 do not commute with e3
        ...
        assert!((final_dist - 0.5).abs() > 1e-9, "ZPE should inject turbulent energy");
    }
}
