...
fn generate_hyper_fractal() -> Result<()> {
    ...
    root.draw_text("Topological Singularities in the G2 Subalgebra (D -> -Inf)", &subfont, (100, 240))?;

    let info_font = ("sans-serif", 35).into_font().color(&RGBColor(180, 180, 180));
    root.draw_text("V(z) = sum exp(i*n*pi/4) / (z^n + epsilon)", &info_font, (100, 320))?;
...
fn generate_mass_ladder() -> Result<()> {
    ...
    let mut chart = ChartBuilder::on(&root)
        .caption("SEDENION VACUUM SPECTRUM: MASS EIGENMODES", ("sans-serif", 100).into_font().color(&WHITE))
        .margin(120)
    ...
    chart.configure_mesh()
        ...
        .axis_desc_style(("sans-serif", 50).into_font().color(&RGBColor(220, 220, 220)))
        .label_style(("sans-serif", 40).into_font().color(&RGBColor(200, 200, 200)))
        .draw()?;
    ...
    root.draw_text("TOV LIMIT", &("sans-serif", 30).into_font().color(&RGBColor(50, 255, 100)), (2500, 2250))?;
...
fn generate_mera_network() -> Result<()> {
    ...
    let depth = 7;
    let leaves = 128;
    let radii: Vec<f64> = (0..=depth).map(|d| 11.0 - d as f64 * (10.0 / depth as f64)).collect();
    ...
    let map_pos = |p: (f64, f64)| -> (i32, i32) {
        (center_x + (p.0 * scale) as i32, center_y + (p.1 * scale) as i32)
    };
    ...
    for d in 1..=depth {
        ...
        for i in 0..positions[d].len() {
            ...
            root.draw(&PathElement::new(vec![map_pos(parent), map_pos(c1)], color.stroke_width(2)))?;
            root.draw(&PathElement::new(vec![map_pos(parent), map_pos(c2)], color.stroke_width(2)))?;
        }
    }
    ...
    for _ in 0..40 {
        ...
        if dist > 18.0 {
            root.draw(&PathElement::new(vec![map_pos(u), map_pos(v)], MAGENTA.mix(0.6).stroke_width(3)))?;
        }
    }

    for (d, layer) in positions.iter().enumerate() {
        for &p in layer {
            let color = if d == 0 { WHITE.mix(1.0) } else { CYAN.mix(0.8) };
    ...
    root.draw_text("SEDENION-ENHANCED BULK GEOMETRY (D = -1.5)", &sub_font, (100, 250))?;
...
fn generate_motif_summary() -> Result<()> {
    ...
    let summary_data: Vec<(f64, f64)> = vec![
        (16.0, 128.0), (32.0, 480.0), (64.0, 1024.0), (128.0, 2048.0), (256.0, 4096.0), (512.0, 8192.0)
    ];

    let mut chart = ChartBuilder::on(&root)
        .caption("CAYLEY-DICKSON MOTIF COMPLEXITY", ("sans-serif", 100).into_font().color(&WHITE))
        .margin(150)
        .x_label_area_size(120)
        .y_label_area_size(180)
    ...
    chart.draw_series(
        summary_data.iter().map(|&(x, y)| {
            Rectangle::new([(x * 0.9, y * 0.9), (x * 1.1, y * 1.1)], CYAN.mix(0.9).filled())
        })
    )?;
...
fn generate_trajectory() -> Result<()> {
    ...
    let mut chart1 = ChartBuilder::on(&top_half[0])
        .caption("ALGEBRAIC RETENTION", ("sans-serif", 60).into_font().color(&WHITE))
        .margin(80).x_label_area_size(80).y_label_area_size(80)
    ...
    let mut chart2 = ChartBuilder::on(&top_half[1])
        .caption("SILICON ISA WIDTH", ("sans-serif", 60).into_font().color(&WHITE))
        .margin(80).x_label_area_size(80).y_label_area_size(80)
        .build_cartesian_2d(1975.0..2025.0, 0.0..1024.0)?;
    chart2.configure_mesh().draw()?;
    let years = [1978.0, 1985.0, 1997.0, 1999.0, 2001.0, 2011.0, 2017.0, 2023.0];
    ...
    let mut _chart3 = ChartBuilder::on(bottom_area)
        .caption("THE HURWITZ CEILING vs SEDENION MELTDOWN", ("sans-serif", 80).into_font().color(&WHITE))
        .margin(100).build_cartesian_2d(0.0..100.0, 0.0..100.0)?;
...
fn generate_dimensional_geometry() -> Result<()> {
    ...
    let mut c1 = ChartBuilder::on(&split[0]).caption("BALL VOLUME V_d(1)", ("sans-serif", 50).into_font().color(&WHITE)).margin(60).build_cartesian_2d(0.0..32.0, -0.5..6.0)?;
    c1.configure_mesh().draw()?;
    c1.draw_series(LineSeries::new(ds.iter().zip(vols.iter()).map(|(&d, &v)| (d, v)), CYAN.stroke_width(3)))?;

    let mut c2 = ChartBuilder::on(&split[1]).caption("SURFACE AREA S_{d-1}", ("sans-serif", 50).into_font().color(&WHITE)).margin(60).build_cartesian_2d(0.0..32.0, -1.0..35.0)?;
    c2.configure_mesh().draw()?;
    c2.draw_series(LineSeries::new(ds.iter().zip(areas.iter()).map(|(&d, &s)| (d, s)), MAGENTA.stroke_width(3)))?;
...
fn generate_4d_mosaic() -> Result<()> {
    ...
    for w in 0..w_tiles {
        for z in 0..z_tiles {
            ...
            for x in 0..20 {
                for y in 0..20 {
                    let val = ((x as f64 - 10.0).powi(2) + (y as f64 - 10.0).powi(2) + (w as f64 * 7.0) + (z as f64 * 3.0)).sin().abs();
                    let color = RGBColor((val * 255.0) as u8, (val * 128.0) as u8, (val * 255.0) as u8);
                    root.draw(&Rectangle::new([(x0 + x * chart_w / 20, y0 + y * chart_h / 20), (x0 + (x+1) * chart_w / 20, y0 + (y+1) * chart_h / 20)], color.filled()))?;
                }
            }
        }
    }
    root.draw_text("4D VACUUM ENTROPY HYPER-MOSAIC", &("sans-serif", 80).into_font().color(&WHITE), (500, 100))?;
    root.present()?;
    println!("Saved.");
    Ok(())
}
...
fn generate_fano_vis() -> Result<()> {
    ...
    if !Path::new(data_path).exists() { return Ok(()); }
    println!("Generating {}...", output_path);
    ...
    for result in reader.records() {
        let r = result?;
        data.push((r[0].parse::<f64>()?, r[1].parse::<f64>()?, r[2].parse::<f64>()?, r[3].parse::<f64>()?));
    }
    let areas = root.split_evenly((3, 1));
    let colors = [CYAN, MAGENTA, GREEN];
    let labels = ["phi=0", "phi=pi/2", "phi=pi"];
    for i in 0..3 {
        let mut chart = ChartBuilder::on(&areas[i]).caption(labels[i], ("sans-serif", 60).into_font().color(&WHITE)).margin(50).build_cartesian_2d(data[0].0..data.last().unwrap().0, 0.0..1.1)?;
        chart.configure_mesh().draw()?;
        chart.draw_series(LineSeries::new(data.iter().map(|d| (d.0, match i { 0=>d.1, 1=>d.2, _=>d.3 })), colors[i].stroke_width(4)))?;
    }
    root.present()?;
    println!("Saved.");
    Ok(())
...
fn generate_zd_projection() -> Result<()> {
    ...
    let mut chart = ChartBuilder::on(&root).caption("SEDENION NILPOTENT MANIFOLD (PCA)", ("sans-serif", 100).into_font().color(&WHITE)).margin(150).build_cartesian_2d(-1.5..1.5, -1.5..1.5)?;
    chart.configure_mesh().draw()?;
