...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let runtime_dims =
        env::var("C590_RUNTIME_BASELINE_DIMS").unwrap_or_else(|_| "128,256".to_string());
    ...
    if env::var("C590_INCLUDE_DIM512").unwrap_or_else(|_| "0".to_string()) == "1"
        && !sweep_dims.contains("512")
    {
        sweep_dims.push_str(",512");
    }
    if env::var("C590_INCLUDE_DIM1024").unwrap_or_else(|_| "0".to_string()) == "1"
        && !sweep_dims.contains("1024")
    {
    ...
    let run_cmd =
        |args: Vec<&str>, envs: Vec<(&str, &str)>| -> Result<(), Box<dyn std::error::Error>> {
