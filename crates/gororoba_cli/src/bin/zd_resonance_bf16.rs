...
fn main() -> Result<()> {
    ...
    let model = match args.model.as_str() {
        "exponential" => ViscosityCouplingModel::Exponential { nu_base, lambda: 2.0 },
        "linear" => ViscosityCouplingModel::Linear { nu_base, alpha: 1.0 },
        ...
        _ => ViscosityCouplingModel::Exponential { nu_base, lambda: 2.0 },
    };
    ...
    solver.set_viscosity_field(&viscosity).context("Failed to upload viscosity")?;
    ...
    for step in 0..args.steps {
        ...
        if step % 1000 == 0 {
            use std::io::Write;
            print!("\rStep {}/{} ({:.1}%)", step, args.steps, step as f64 / args.steps as f64 * 100.0);
            std::io::stdout().flush()?;
        }
    }
