...
struct Args {
    ...
    #[arg(long, default_value = "AMD Ryzen 9 7950X / 64 GB DDR5 / NVIDIA RTX 4090")]
    hardware: String,
}
...
fn generate_thesis_results_table(summary: &SynthesisSummary, t2_data: Option<&T2PowerLawData>) -> String {
    let mut out = String::new();
    ...
    if let Some(data) = t2_data.filter(|d| !d.series.is_empty()) {
            ...
            out.push_str("\\caption{Power-law viscosity parameter sweep (T2).}\\label{tab:t2-sweep}\\\\\n");
            out.push_str("\\toprule\n");
            ...
            for s in &data.series {
                let nn = s.non_newtonian.map_or("--", |b| if b { "Yes" } else { "No" });
                let ratio = s.slope_ratio.map_or("--".to_string(), |r| format!("{r:.4}"));
                out.push_str(&format!(
                    "{:.3} & {:.3} & {:.3} & {} & {} \\\\\n",
...
fn generate_reproducibility_appendix(
    ...
    if let Some(m) = manifest {
        ...
        if !m.artifact.is_empty() {
            ...
            out.push_str("\\toprule\n");
            out.push_str("\\textbf{Artifact} & \\textbf{SHA-256 (first 16)} & \\textbf{Size} \\\\\n");
            out.push_str("\\midrule\n");
            out.push_str("\\endfirsthead\n");
            out.push_str("\\toprule\n");
            out.push_str("\\textbf{Artifact} & \\textbf{SHA-256 (first 16)} & \\textbf{Size} \\\\\n");
...
fn main() {
    ...
    if args.repro_appendix {
        ...
        let tex = generate_reproducibility_appendix(
            &experiments,
            manifest.as_ref(),
            &args.hardware,
        );
