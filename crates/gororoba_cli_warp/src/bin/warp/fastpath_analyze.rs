...
#[cfg(feature = "hdf5-export")]
fn run_inner(argv: &[String]) -> Result<(), Box<dyn Error>> {
...
fn run_inner(_argv: &[String]) -> Result<(), Box<dyn Error>> {
    Err(std::io::Error::other(
        "warp fastpath-analyze requires the hdf5-export feature: cargo run -p gororoba_cli_warp --features 'gpu,hdf5-export' --bin warp -- fastpath-analyze <production_dir>",
    )
    .into())
}
