...
#[cfg(feature = "hdf5-export")]
fn parse_args(argv: &[String]) -> Result<CliArgs, Box<dyn Error>> {
...
pub fn run(argv: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    {
        // Only the feature-on arm parses arguments, so bind the slice here to
        // keep one signature across both configurations.
        let _ = argv;
        Err(std::io::Error::other(
            "warp acceptance-gate requires the hdf5-export feature: cargo run -p gororoba_cli_warp --features hdf5-export --bin warp -- acceptance-gate <paths>",
        )
        .into())
    }
}
