use anyhow::Result;
use clap::Parser;
use gr_core::warp_metric::{NacelleWarpBubble, NacelleWarpParams, nacelle_energy_density};

#[derive(Parser, Debug)]
#[command(
    author,
    version,
    about = "Dimensional Phase-Transition Sweep for Nacelle Holography"
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    for (dim, ratio) in dimensions.iter().zip(cd_ratios.iter()) {
        ...
        println!(
            "CD Dim: {:2} | Nacelles: {:2} | CD Ratio: {:.1} | Peak Stress-Energy: {:.6e}",
            dim, n_nacelles, ratio, rho_val
        );

        if n_nacelles == 16 && *ratio > 0.1 {
            println!(
                "  => Phase transition detected: Stress-energy localized into 16 discrete nacelles!"
            );
        }
    }
