//! Imbalance Energy: Map graph-theoretic imbalance to physical energy scales.
//!
//! This module connects the Harary-Zaslavsky imbalance index F (fraction of
//! unbalanced triangles) to physical energy E_imbalance, providing the
...
//! - Imbalance index: F = (# unbalanced) / (# total triangles)
//!
//! Physical interpretation:
//! - Each unbalanced triangle carries energy cost E_0 (bond mismatch energy)
//! - Total imbalance energy: E_frust = F * C(N,3) * E_0
//! - Imbalance attractor F = 3/8: local energy minimum state
//!
//! # References
//!
...
#[derive(Debug, Clone)]
pub struct ImbalanceEnergy {
    /// Imbalance index F in [0, 1]
    ...
    /// Total imbalance energy (Joules)
    pub total_energy_j: f64,
    /// Total imbalance energy (eV)
    pub total_energy_ev: f64,
    /// Deviation from imbalance attractor (F - 3/8)
...
const IMBALANCE_ATTRACTOR: f64 = 0.375; // 3/8
...
/// Compute imbalance energy from signed graph structure.
///
/// # Arguments
/// * `imbalance` - Harary-Zaslavsky imbalance index F in [0, 1]
/// * `n_vertices` - Number of vertices in signed graph
/// * `e0_joules` - Energy per unbalanced triangle (material-dependent)
///
/// # Returns
/// ImbalanceEnergy struct with energy breakdown
///
/// # Example
/// ```ignore
/// let imbalance = 0.375; // Imbalance attractor
/// let n_vertices = 16; // Sedenion dimension
/// let e0 = 1e-21; // ~6 meV per triangle
/// let energy = compute_imbalance_energy(imbalance, n_vertices, e0);
/// ```
pub fn compute_imbalance_energy(
    imbalance: f64,
    n_vertices: usize,
    e0_joules: f64,
) -> ImbalanceEnergy {
    ...
    // Deviation from imbalance attractor
    let vacuum_deviation = imbalance - IMBALANCE_ATTRACTOR;
...
/// 3. Physical interpretation: Associator norm quantifies imbalance energy in algebraic structure
...
pub fn estimate_e0_from_associators(dim: usize) -> f64 {
    ...
    // Typical bond disorder/imbalance energy in atomic/molecular systems
    const REFERENCE_ENERGY_J: f64 = 1.602e-22; // 1 meV

    // E_0 = dimensionless associator norm * reference energy
    // This gives a TEMPERATURE-INDEPENDENT imbalance energy scale
    mean_norm * REFERENCE_ENERGY_J
}

/// Compute lambda coupling strength from imbalance energy and thermal energy.
///
/// # Arguments
/// * `e_imbalance` - Imbalance energy (Joules)
/// * `temperature_k` - Temperature (Kelvin)
///
/// # Returns
/// Dimensionless lambda = E_imbalance / (k_B * T)
///
/// # Interpretation
/// - lambda >> 1: Imbalance dominates thermal fluctuations (strong coupling)
/// - lambda ~ 1: Imbalance and thermal energy comparable (moderate coupling)
/// - lambda << 1: Thermal fluctuations dominate (weak coupling)
pub fn compute_lambda(e_imbalance: f64, temperature_k: f64) -> f64 {
    let e_thermal = K_BOLTZMANN * temperature_k;
    e_imbalance / e_thermal
}

/// Predict lambda for Sedenion field at given temperature.
///
/// # Arguments
/// * `imbalance` - Harary-Zaslavsky imbalance index
...
/// 2. Compute total imbalance energy: E_frust = F * C(16,3) * E_0
...
pub fn predict_lambda_sedenion(imbalance: f64, temperature_k: f64) -> f64 {
    ...
    // Compute imbalance energy
    let frust_energy = compute_imbalance_energy(imbalance, DIM, e0);
...
mod tests {
    ...
    #[test]
    fn test_imbalance_energy_imbalance_attractor() {
        let imbalance = 0.375; // 3/8 imbalance attractor
        let n_vertices = 16; // Sedenion
        let e0 = 1e-21; // ~6 meV

        let energy = compute_imbalance_energy(imbalance, n_vertices, e0);

        assert_eq!(energy.imbalance_index, 0.375);
    ...
    #[test]
    fn test_imbalance_energy_extremes() {
        let e0 = 1e-21;

        // F = 0: all balanced (minimum energy)
        let e_min = compute_imbalance_energy(0.0, 16, e0);
        assert_eq!(e_min.total_energy_j, 0.0);

        // F = 1: all unbalanced (maximum energy)
        let e_max = compute_imbalance_energy(1.0, 16, e0);
    ...
    fn test_compute_lambda_low_temperature() {
        let e_imbalance = 1e-21; // ~6 meV
        let temperature = 4.2; // He-4 critical temperature

        let lambda = compute_lambda(e_imbalance, temperature);

        // At low T, lambda should be large (imbalance dominates)
    ...
    fn test_predict_lambda_sedenion_he4() {
        let imbalance = 0.35; // Typical imbalanced value
        let temperature = 4.2; // He-4 at lambda point

        let lambda = predict_lambda_sedenion(imbalance, temperature);

        // Lambda is TEMPERATURE-DEPENDENT: lambda = E_frust / (k_B*T)
        // He-4 at 4.2 K: low temperature -> high lambda (imbalance dominates)
    ...
    #[test]
    fn test_imbalance_attractor_minimum_energy() {
        let e0 = 1e-21;
        let n_vertices = 16;

        // Imbalances around imbalance attractor
        let imbalances = [0.30, 0.35, 0.375, 0.40, 0.45];
        let energies: Vec<f64> = imbalances
            .iter()
            .map(|&f| compute_imbalance_energy(f, n_vertices, e0).total_energy_j)
            .collect();

        // Energy should be minimized near F = 3/8 (but NOT exactly at 3/8)
        // Actually F=0 is true minimum, but 3/8 is imbalance attractor
        let e_vacuum = compute_imbalance_energy(0.375, n_vertices, e0).total_energy_j;

        // Deviation energy increases quadratically
        for (i, &f) in imbalances.iter().enumerate() {
