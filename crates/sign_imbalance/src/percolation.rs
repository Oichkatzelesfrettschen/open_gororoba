//! Percolation channel detection and imbalance correlation.
//!
//! This module implements algorithms to:
//! 1. Detect percolation channels in 3D velocity fields (BFS-based)
//! 2. Correlate channel distribution with imbalance density (Welch's t-test)
//! 3. Compute channel statistics for analysis
//!
//! Used in E-027 experiment to validate Thesis 1 (imbalance-viscosity coupling).
...
/// Result of percolation-imbalance correlation analysis.
#[derive(Clone, Debug)]
pub struct CorrelationResult {
    ...
    pub effect_size: f64,
    /// Mean imbalance in channels
    pub mean_imbalance_channels: f64,
    /// Mean imbalance in background (non-channel cells)
    pub mean_imbalance_background: f64,
...
/// Correlate percolation channels with imbalance density.
///
/// Uses Welch's t-test to compare imbalance values in channel cells
/// vs background cells.
pub fn correlate_with_imbalance(
    ...
    if imbalance_field.is_empty() {
        return CorrelationResult {
            ...
            p_value: 1.0,
            effect_size: 0.0,
            mean_imbalance_channels: f64::NAN,
            mean_imbalance_background: f64::NAN,
            n_channel: 0,
            n_background: 0,
        };
    }
    ...
    // Mark channel cells
    let mut is_channel = vec![false; imbalance_field.len()];
    let mut channel_imbalances = Vec::new();

    // Infer grid size from imbalance field length (assuming cubic grid)
    let n_cells = imbalance_field.len();
    ...
    let mut background_imbalances = Vec::new();
    for (i, &f) in imbalance_field.iter().enumerate() {
        if is_channel[i] {
            channel_imbalances.push(f);
        } else {
            background_imbalances.push(f);
        }
    }

    if channel_imbalances.is_empty() || background_imbalances.is_empty() {
        return CorrelationResult {
            t_statistic: f64::NAN,
            p_value: 1.0,
            effect_size: 0.0,
            mean_imbalance_channels: f64::NAN,
            mean_imbalance_background: f64::NAN,
            n_channel: channel_imbalances.len(),
            n_background: background_imbalances.len(),
        };
    }
    ...
    let var_ch = channel_imbalances
        ...
        .sum::<f64>()
        / (channel_imbalances.len() as f64);
    let var_bg = background_imbalances
        .iter()
        .map(|&f| (f - mean_bg).powi(2))
        .sum::<f64>()
        / (background_imbalances.len() as f64);

    // Welch's t-test
    let n_ch = channel_imbalances.len() as f64;
    let n_bg = background_imbalances.len() as f64;
    ...
    CorrelationResult {
        ...
        p_value,
        effect_size,
        mean_imbalance_channels: mean_ch,
        mean_imbalance_background: mean_bg,
        n_channel: channel_imbalances.len(),
        n_background: background_imbalances.len(),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_correlation_with_imbalance() {
        ...
        let imbalance_field = vec![0.3; 4096]; // Uniform imbalance
        let result = correlate_with_imbalance(&[channel], &imbalance_field);
