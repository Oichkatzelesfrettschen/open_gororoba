...
use optics_core::{AlgebraicLensingGpu, AlgebraicLensingGpuConfig, GpuVec3};
...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg(feature = "gpu")]
    {
        ...
        println!(
            "Tracing {} million rays...",
            (initial_pos.len() as f32 / 1e6)
        );

        let alpha = 10.0; // Strong lensing
        let dt = 0.1;
        let max_steps = 1000;
        let config = AlgebraicLensingGpuConfig::new(nx, ny, nz, alpha, dt, max_steps);

        let start = Instant::now();
        let (final_pos, _final_dir) =
            tracer.trace_rays(config, &density, &initial_pos, &initial_dir)?;
        let duration = start.elapsed();

        println!(
            "Done! Time: {:?}, Rays/sec: {:.2} million",
            duration,
