...
// Thermal dynamics extension lives in the `thermal` submodule.
// Re-exports below preserve the public API surface
// (`optics_core::tcmt::ThermalCavity`, `ThermalTcmtSolver`, ...).
pub mod thermal;
pub use thermal::{
    ThermalCavity, ThermalCavityState, ThermalRegime, ThermalSteadyStateResult, ThermalTcmtSolver,
    thermal_regime,
};

// Hysteresis detection, structured TcmtError, and cavity-parameter
// validation live in the `hysteresis` submodule.
pub mod hysteresis;
pub use hysteresis::{
    HysteresisResult, HysteresisTrace, TcmtError, TurningPoint, TurningPointBranch,
    find_turning_points, find_turning_points_physical, hysteresis_width, trace_hysteresis_loop,
    validate_cavity,
};
