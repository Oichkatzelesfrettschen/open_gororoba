//! Numerical QGP R_AA score replay with explicit source admission.
//!
//! Retained CUJET and Langevin arrays have unresolved figure provenance.
//! Their parameter penalties reproduce declared scores, not authenticated fits.
//! Physical ranking requires source, population, covariance and fit admission.
//!
...
/// Check source identity and population before the retained comparison runs.
pub fn retained_bic_population_admission() -> Result<(), String> {
    ...
    let source_population = ReferencePopulation {
        energy_gev: 2760,
        centrality_percent: (20, 30),
        ..measured.clone()
    };
    let mut failures: Vec<_> = [cujet3_pbpb_2760_0_5(), langevin_pbpb_5020_0_10()]
        .iter()
        .filter_map(|curve| curve.source.require_identified_vector().err())
        .collect();
    if let Err(error) = require_matched_population(&measured, &source_population) {
        failures.push(format!("CUJET3.0 Figure 3(a): {error}"));
    }
...
/// Provenance of a numerical vector, independently of its legacy display name.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CurveSource {
    /// Arrays attributed to CUJET figures that contain transport coefficients.
    QuarantinedCujet,
    /// Arrays attributed to a displacement figure in a static charm study.
    QuarantinedLangevin,
    /// Values generated by a declared numerical function or test fixture.
    Constructed,
}

impl CurveSource {
    /// Reject unidentified published vectors even when population labels match.
    /// Constructed values pass computational provenance only; physical
    /// population, covariance and fitting-budget checks remain independent.
    pub fn require_identified_vector(self) -> Result<(), String> {
        match self {
            Self::QuarantinedCujet => Err(
                "CUJET3.0 source admission failed: Xu, Liao and Gyulassy, arXiv:1508.00552v3, Figures 4/5 contain transport coefficients; Figure 3(a) contains charged-hadron RAA at Pb-Pb 2.76 TeV, 20-30%; retained vectors lack authenticated extraction".into(),
            ),
            Self::QuarantinedLangevin => Err(
                "fractional Langevin source admission failed: Jai Prakash, arXiv:2401.03757, Figure 3 is mean squared displacement; Figure 4 contains static charm-quark RAA at 6 fm/c and 250/350 MeV; collision energy, particle species mapping to D mesons and centrality are unsupported for retained vectors".into(),
            ),
            Self::Constructed => Ok(()),
        }
    }
...
/// A numerical R_AA vector with explicit computational provenance.
#[derive(Debug, Clone)]
pub struct TheoreticalRaaCurve {
    /// Model name.
    pub name: &'static str,
    /// Source admission is independent of legacy names and score penalties.
    pub source: CurveSource,
    /// Declared score penalty; quarantine preserves unverified legacy counts.
    pub n_params: usize,
...
// ============================================================================
// Retained numerical vectors with unresolved published-figure identity
// ============================================================================

/// Quarantined vector carrying the legacy Pb-Pb 2.76 TeV, 0-5% label.
/// Xu, Liao and Gyulassy, JHEP02(2016)169, Figure 4 contains transport
/// coefficients. Figure 3(a) R_AA uses 20-30%; relabeling cannot identify
/// the retained numbers. The five-parameter penalty is a replay convention.
pub fn cujet3_pbpb_2760_0_5() -> TheoreticalRaaCurve {
    TheoreticalRaaCurve {
        name: "CUJET3.0 Pb-Pb 2.76 TeV 0-5%",
        source: CurveSource::QuarantinedCujet,
        n_params: 5,
        pt: vec![
...
/// Quarantined vector carrying the legacy Pb-Pb 2.76 TeV, 10-20% label.
/// JHEP02(2016)169 Figure 4 is a transport-coefficient plot.
...
/// Quarantined vector carrying the legacy Pb-Pb 2.76 TeV, 30-40% label.
/// JHEP02(2016)169 Figure 5 is a transport-coefficient plot.
...
/// Quarantined vector carrying the legacy Pb-Pb 5.02 TeV, 0-10% label.
/// Jai Prakash, arXiv:2401.03757, Figure 3 shows displacement; Figure 4
/// shows static charm-quark R_AA at 6 fm/c and 250/350 MeV. The source
/// supplies neither the labeled D-meson population nor its centrality.
/// The three-parameter penalty is an unverified replay convention.
pub fn langevin_pbpb_5020_0_10() -> TheoreticalRaaCurve {
    TheoreticalRaaCurve {
        name: "Frac. Langevin Pb-Pb 5.02 TeV 0-10%",
        source: CurveSource::QuarantinedLangevin,
        n_params: 3,
        pt: vec![
...
/// Quarantined vector carrying the legacy Pb-Pb 5.02 TeV, 30-50% label.
/// Prakash's arXiv:2401.03757 static-medium Figure 4 lacks centrality bins.
...
pub fn arleo_falmagne_raa(
    ...
    TheoreticalRaaCurve {
        name: "Arleo-Falmagne",
        source: CurveSource::Constructed,
        n_params: 1,
        pt: pt_points.to_vec(),
        raa,
    }
