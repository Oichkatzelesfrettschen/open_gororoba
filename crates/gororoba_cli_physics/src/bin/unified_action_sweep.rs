...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let target_dims: Vec<usize> = vec![
        16, 32, 64, 512, 4096, 65536, 1_048_576, 1_073_741_824
    ];
    ...
    for &dim in &target_dims {
        ...
        for i in 0..=steps {
            ...
            writeln!(file, "{},{:.6},{:.6},{:.6},{:.6}", dim, phi, l_grav, l_avt, action)?;
        }
        ...
        if shift > 0.0 {
            ...
            println!("   - Status: LOCKED. Exact equivalence achieved due to overwhelming topological friction.");
        } else {
            println!("   - Status: FAILED. Unexpected positive shift.");
        }
    }

    println!("\n✅ Sweep complete. High-precision data written to {}", out_path.display());
