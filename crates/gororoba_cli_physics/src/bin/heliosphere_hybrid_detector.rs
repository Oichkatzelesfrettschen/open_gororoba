...
struct Cli {
    ...
    #[arg(long, default_value = "data/output/heliosphere/ablations/hybrid_detector_results.json")]
    out_json: PathBuf,
...
fn main() -> Result<()> {
        ...
        if let Ok(content) = fs::read_to_string(&path) {
            for r in parse_themis_fgm_hapi_csv_minutes(&content, &probe_upper) {
                if r.b_magnitude <= cli.max_bmag {
                    let day_offset =
                        (r.year as f64 - start.year() as f64) * 365.25 + (r.doy as f64 - start.ordinal() as f64);
    ...
    let pvi_h: Vec<f64> = pvi_crossings(&bx, &by, &bz, 1, 4.0, 10)
        .iter().map(|&i| elapsed_hrs[i.min(n - 1)]).collect();
    ...
    println!("  Detectors: {}", all_detectors.iter().map(|(n, h)| format!("{}={}", n, h.len())).collect::<Vec<_>>().join(", "));

    // Helper: compute detection/FA/F1 for a set of detected hours
    let score = |det_hours: &[f64]| -> (f64, f64, f64, usize) {
        let matched = curated_hours.iter()
            .filter(|&&ch| det_hours.iter().any(|&dh| (dh - ch).abs() < tol))
            .count();
        let rev_matched = det_hours.iter()
            .filter(|&&dh| curated_hours.iter().any(|&ch| (ch - dh).abs() < tol))
            .count();
        let det_rate = matched as f64 / nc.max(1) as f64;
        let fa_rate = if det_hours.is_empty() { 0.0 }
            else { (det_hours.len() - rev_matched) as f64 / det_hours.len() as f64 };
        let precision = if det_hours.is_empty() { 0.0 } else { rev_matched as f64 / det_hours.len() as f64 };
        let recall = det_rate;
    ...
    for &(name, hours) in &all_detectors {
        ...
        individual.push(HybridResult {
            combination: name.to_string(), n_detections: nd,
            detection_rate: det, false_alarm_rate: fa, f1_score: f1,
        });
    }
    ...
    let triple_combos: Vec<(&str, Vec<f64>)> = vec![
        ...
        ("CD+Shear+Entropy", merge(&merge(&cd_hours, &shear_h), &ent_h)),
    ...
    unions.push(HybridResult {
        combination: "ALL-6 UNION".to_string(), n_detections: nd,
        detection_rate: det, false_alarm_rate: fa, f1_score: f1,
    });
    ...
    for min_agree in 1..=6 {
        ...
        for &ch in &curated_hours {
            let count = all_detectors.iter()
                .filter(|&&(_, hours)| hours.iter().any(|&dh| (dh - ch).abs() < tol))
                .count();
            if count >= min_agree { detected += 1; }
        }
    ...
    // Print results
    println!("\n=== Individual Detectors ===");
    println!("{:<25} {:>6} {:>8} {:>8} {:>6}", "Detector", "N", "Det%", "FA%", "F1");
    println!("{}", "-".repeat(55));
    for r in &individual {
        println!("{:<25} {:>6} {:>7.1}% {:>7.1}% {:>.3}", r.combination, r.n_detections, r.detection_rate*100.0, r.false_alarm_rate*100.0, r.f1_score);
    }

    println!("\n=== Union Combinations ===");
    println!("{:<25} {:>6} {:>8} {:>8} {:>6}", "Combination", "N", "Det%", "FA%", "F1");
    println!("{}", "-".repeat(55));
    // Sort by F1
    ...
    println!("\n=== Agreement Thresholds ===");
    for r in &agreement {
        println!("  {}: {}/{} = {:.1}%", r.combination, r.n_detections, nc, r.detection_rate*100.0);
    }
