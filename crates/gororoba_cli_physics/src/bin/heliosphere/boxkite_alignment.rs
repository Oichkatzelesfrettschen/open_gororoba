...
pub fn run(cli: Cli) -> Result<()> {
    ...
    let mut reader = ReaderBuilder::new().from_path(&cli.input_csv)?;
    ...
    let target_backend = cli.backend.as_ref().map(|s| s.to_lowercase());
    ...
    println!("[4/4] Writing results to {}...", cli.out_csv.display());
