...
fn main() -> io::Result<()> {
    ...
    for &dim in &cd_dims {
        ...
        println!("    Imbalance index: {:.4}", imbalance);
    ...
    // -----------------------------------------------------------------------
    // Section 4: CD vs J1-J2 imbalance-matched comparison
    // -----------------------------------------------------------------------
    println!("[4/4] Imbalance-matched comparison: CD vs J1-J2");
    println!("-------------------------------------------------");

    for &dim in &cd_dims {
        let cd_model = build_cd_heisenberg(dim, 0.0);
        let f_cd = graph_imbalance_index(&cd_model);
        let cd_transport = dispatcher.compute(&cd_model, 0.5);

        // Find J1-J2 alpha with matching imbalance
        ...
        let f_matched = graph_imbalance_index(&matched_chain);
