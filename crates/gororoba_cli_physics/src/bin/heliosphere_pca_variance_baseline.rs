...
fn pca_leading_variance_ratio(window: &[Vec<f64>], dim: usize, n_iter: usize) -> f64 {
    ...
    let total_var: f64 = window.iter().flat_map(|v| {
        v.iter().enumerate().map(|(j, &val)| (val - mean[j]).powi(2))
    }).sum::<f64>();

    if total_var < 1e-30 { return 1.0; }
    ...
    for _ in 0..n_iter {
        ...
        for v in window.iter() {
            ...
            let dot: f64 = v.iter().enumerate().map(|(j, &val)| (val - mean[j]) * pc[j]).sum();
            // next += dot * (v - mu)
        ...
        if norm < 1e-30 { break; }
        for val in next.iter_mut() { *val /= norm; }
        pc = next;
    }

    // Variance explained by leading PC = (1/n) sum_i ((v_i - mu)^T pc)^2
    let var_pc1: f64 = window.iter().map(|v| {
        let dot: f64 = v.iter().enumerate().map(|(j, &val)| (val - mean[j]) * pc[j]).sum();
        dot * dot
    }).sum::<f64>() / n as f64;
...
fn main() -> Result<()> {
    ...
    let pca_scores: Vec<f64> = (0..delay_vectors.len()).map(|t| {
        if t + 1 < pca_window { return 0.0; }
    ...
    if pca_scores.len() > trans_window * 2 {
        ...
        let global_std: f64 = {
            let var = pca_scores.iter().map(|&a| (a - global_mean).powi(2)).sum::<f64>()
        ...
        for i in half..pca_scores.len().saturating_sub(half) {
            let pre_mean: f64 = pca_scores[i.saturating_sub(half)..i].iter().sum::<f64>()
                / half as f64;
            let post_mean: f64 = pca_scores[i..(i + half).min(pca_scores.len())]
                .iter().sum::<f64>() / half.min(pca_scores.len() - i) as f64;
    ...
    let results = PcaVarianceResults {
        ...
        pca_window,
        method: format!("PCA leading-PC variance ratio (dim={}, pca_window={}, power_iter={}, MAD_SCALE_FACTOR=1.5)",
