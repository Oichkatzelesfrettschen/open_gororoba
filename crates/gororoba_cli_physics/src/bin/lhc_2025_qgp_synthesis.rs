...
struct Args {
    ...
    #[arg(
        long,
        default_value = "data/external/lhc_2025/cms/p_pb_8.16tev_azimuthal.csv"
    )]
    cms_data: String,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mean_alice_p = if !alice_predictions.is_empty() {
        alice_predictions.iter().sum::<f64>() / alice_predictions.len() as f64
    } else {
        0.0
    };
    let mean_cms_residual = if !cms_residuals.is_empty() {
        cms_residuals.iter().map(|x| x.abs()).sum::<f64>() / (cms_residuals.len() as f64)
    } else {
        1.0
    };
    let report = format!(
...
fn load_cms_data(path: &str) -> Result<Vec<CmsRecord>, Box<dyn Error>> {
    ...
    for result in rdr.records() {
        let record = result?;
        results.push(CmsRecord {
            phi_rad: record[0].parse()?,
            p_z_val: record[1].parse()?,
            hydro_model_val: record[3].parse()?,
        });
    }
    Ok(results)
}
