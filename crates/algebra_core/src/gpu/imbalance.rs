//! GPU-accelerated imbalance ratio computation via hybrid CPU-BFS + GPU validation.
//!
//! The imbalance ratio measures the fraction of edges that violate the coboundary
...
#[cfg(feature = "gpu")]
const IMBALANCE_KERNEL_SRC: &str = r#"
...
/// GPU-accelerated imbalance computation.
pub struct ImbalanceGpu;
...
impl ImbalanceGpu {
    /// Compute imbalance ratio using hybrid CPU-BFS + GPU edge validation.
    ///
    /// # Arguments
    /// * `edges` - List of edges
    ...
    pub fn compute_imbalance_gpu(
        ...
        #[cfg(feature = "gpu")]
        {
            ...
            if let Ok(result) = Self::compute_imbalance_hybrid(edges, n_nodes, eta_values) {
                return Ok(result);
            }
        }

        // Fall back to CPU
        Self::compute_imbalance_cpu(edges, n_nodes, eta_values)
    }

    /// Hybrid CPU-BFS + GPU edge validation.
    #[cfg(feature = "gpu")]
    fn compute_imbalance_hybrid(
        edges: &[(usize, usize)],
        n_nodes: usize,
        eta_values: &[u8],
    ) -> Result<ImbalanceResult, String> {
        ...
        let imbalance_ratio = frustrated_count as f64 / cycle_rank.max(1) as f64;
    ...
    /// CPU fallback: compute imbalance via BFS.
    pub fn compute_imbalance_cpu(
        edges: &[(usize, usize)],
        n_nodes: usize,
        eta_values: &[u8],
    ) -> Result<ImbalanceResult, String> {
...
mod tests {
    ...
    #[test]
    fn test_cpu_imbalance_simple() {
        // Simple test: line graph (no cycles, no imbalance)
        let edges = vec![(0, 1), (1, 2), (2, 3)];
        let eta_values = vec![0, 0, 0]; // All zeros
        let n_nodes = 4;

        let result = ImbalanceGpu::compute_imbalance_cpu(&edges, n_nodes, &eta_values);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_cpu_imbalance_triangle() {
        // Triangle graph (1 cycle)
        let edges = vec![(0, 1), (1, 2), (2, 0)];
        let eta_values = vec![0, 0, 1]; // Last edge has eta=1, causes imbalance
        let n_nodes = 3;
        ...
        eprintln!(
            ...
            result.frustrated_count, result.cycle_rank, result.imbalance_ratio
        );
        assert!(
            result.frustrated_count > 0,
            "Triangle should have imbalance"
        );
    }

    #[cfg(feature = "gpu")]
    #[test]
    fn test_gpu_imbalance_simple() {
        ...
        let result = ImbalanceGpu::compute_imbalance_gpu(&edges, n_nodes, &eta_values);
        assert!(result.is_ok());
    ...
    fn test_gpu_vs_cpu_imbalance() {
        ...
        // CPU result
        let cpu_result = ImbalanceGpu::compute_imbalance_cpu(&edges, n_nodes, &eta_values)
            .expect("CPU computation");

        // GPU result
        let gpu_result = ImbalanceGpu::compute_imbalance_gpu(&edges, n_nodes, &eta_values)
            .expect("GPU computation");

        // Verify results match
        assert_eq!(
            cpu_result.frustrated_count, gpu_result.frustrated_count,
            "GPU and CPU imbalance counts must match"
        );
        eprintln!(
