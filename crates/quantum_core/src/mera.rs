...
pub fn random_unitary(d: usize, rng: &mut ChaCha8Rng) -> ComplexMatrix {
    ...
    for row in &mut a {
        for elem in row.iter_mut() {
            *elem = Complex64::new(rng.random_range(-1.0..1.0), rng.random_range(-1.0..1.0));
        }
    }
...
fn gram_schmidt_orthonormalize(a: &mut ComplexMatrix) {
    ...
    for i in 0..d {
        ...
        for j in 0..i {
            ...
            let basis_j = a[j].clone();
            for (k, elem) in a[i].iter_mut().enumerate() {
                *elem -= proj * basis_j[k];
            }
        }
...
pub fn mera_entropy_estimate(subsystem_size: usize, d: usize, seed: u64) -> f64 {
    ...
    let mut eigenvalues: Vec<f64> = (0..dim).map(|_| rng.random::<f64>()).collect();
