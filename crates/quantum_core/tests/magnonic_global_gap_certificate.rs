...
fn write_new(path: &Path, contents: &str) {
    let mut file = std::fs::File::create_new(path).expect("Preserve existing retained artifacts");
...
fn retain_table_i_then_table_ii_global_certificate() {
    ...
    let mut report_file = std::fs::File::create_new(destination.join("results.toml"))
        .expect("Use a fresh destination for each retained campaign");
