...
use verified_core::coupler_manifold::{
    CouplerJacobian, CouplerPoint, IdentifiabilityAudit, mipt::effective_measurement_rate,
    qec::TwoSectorMixture,
};
...
fn main() {
    ...
    for p in p_vals {
        ...
        println!(
            "p={:.2}: eps_smooth={:.2e}, eps_obs={:.2e}",
            p, eps_smooth, eps_obs
        );
    }
    ...
    println!(
        "Jacobian (Density Path) [0,0] (d ln O / d ln p): {:.2}",
        j_density.j_mat[(0, 0)]
    );
    println!(
        "Jacobian (Depth Path)   [0,0] (d ln O / d ln p): {:.2}",
        j_depth.j_mat[(0, 0)]
    );
    println!(
        "Mismatch {:.2} vs {:.2} indicates confounding from T-dependent noise.",
        j_density.j_mat[(0, 0)],
        j_depth.j_mat[(0, 0)]
    );
    ...
    println!(
        "Identifiability Audit (Depth Path): Identifiable={}, Cond={:.2e}",
    ...
    println!("d(2, 4) = {:.2}", d24);
    println!(
        "Strong Triangle Inequality: d(0,4) <= max(d(0,2), d(2,4)) -> {:.2} <= {:.2}",
        d04,
        d02.max(d24)
    );
}
