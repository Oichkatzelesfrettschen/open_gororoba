...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("--- Manifold Projection: Residualized Astrophysical Catalogs ({}) ---", path);
    ...
    let valid_points: Vec<_> = points_data.into_iter()
    ...
    for i in 0..(manifold_points.len() - 1) {
        ...
        if (manifold_points[i+1].g[0] - manifold_points[i].g[0]).abs() > 0.5 {
            if let Ok(jac) = CouplerJacobian::estimate_from_delta(&manifold_points[i], &manifold_points[i+1]) {
                let j_val = jac.j_mat[(0,0)];
                println!("  Transition: {} -> {}", sorted_points[i].dataset, sorted_points[i+1].dataset);
                println!("  g: {:.0} -> {:.0}", manifold_points[i].g[0], manifold_points[i+1].g[0]);
                println!("  J: {:.4}\n", j_val);
    ...
    if !cross_jacobians.is_empty() {
        ...
        let variance = cross_jacobians.iter().map(|j| (j - mean_j).powi(2)).sum::<f64>() / cross_jacobians.len() as f64;
        ...
        if variance > 1.0 {
            println!("!! INSIGHT: High variance indicates that removing instrument bias shatters the 'universal' scaling.");
            println!("The previous tight variance may have been an artifact of shared systemic measurement noise (the confounder) rather than deep physical universality.");
        } else {
            println!("!! INSIGHT: Tight variance persists even after residualizing instrument bias.");
            println!("This strengthens the thesis that information-theoretic bounds are fundamentally geometric.");
        }
    } else {
