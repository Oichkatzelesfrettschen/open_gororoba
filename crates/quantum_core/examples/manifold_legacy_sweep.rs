...
fn extract_csv_points(path: &str, g_col: usize, o_col: usize, skip: usize, filter: Option<&str>) -> Vec<CouplerPoint> {
...
fn main() {
    ...
    let pts1 = extract_csv_points("../../data/csv/legacy/sedenion_field_metrics_4D.csv", 1, 2, 1, None);
    let j1 = calculate_mean_jacobian(&pts1);
    println!("   Mean Jacobian <J>: {:.4}", j1);
    println!("   Insight: J ~ 0.5. The thermodynamic energy of the field scales as the square root of the non-associativity (Associator). The breakdown of algebraic structure is thermodynamically expensive, but scales sub-linearly.\n");
    ...
    let p_mod1 = CouplerPoint { g: DVector::from_vec(vec![0.1]), o: DVector::from_vec(vec![174537.9]) };
    let p_mod2 = CouplerPoint { g: DVector::from_vec(vec![0.45]), o: DVector::from_vec(vec![187794.8]) };
    let j_mod = CouplerJacobian::estimate_from_delta(&p_mod1, &p_mod2).unwrap().j_mat[(0,0)];
    println!("   Modular Transformation Jacobian J: {:.4}", j_mod);
    println!("   Insight: The modular transformation maps shifts in the upper half plane into extreme high-magnitude topological invariants, exhibiting highly resilient (J ~ 0.05) boundaries.\n");
    ...
    let p_norm1 = CouplerPoint { g: DVector::from_vec(vec![1.0]), o: DVector::from_vec(vec![23.8624]) };
    let p_norm2 = CouplerPoint { g: DVector::from_vec(vec![2.0]), o: DVector::from_vec(vec![23.8118]) };
    let p_norm3 = CouplerPoint { g: DVector::from_vec(vec![3.0]), o: DVector::from_vec(vec![25.4831]) };
    let j_norm1 = CouplerJacobian::estimate_from_delta(&p_norm1, &p_norm2).unwrap().j_mat[(0,0)];
    let j_norm2 = CouplerJacobian::estimate_from_delta(&p_norm2, &p_norm3).unwrap().j_mat[(0,0)];
    println!("   Norm Scaling Jacobian J: {:.4}", (j_norm1 + j_norm2) / 2.0);
    println!("   Insight: Even with massive element vectors, the norm operations are highly stable and scale sub-linearly (J ~ 0.08), validating the bounded nature of the Sedenion unit sphere despite non-associativity.\n");
}
