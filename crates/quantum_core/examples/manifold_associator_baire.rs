...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("--- Domain 4: CD-Algebra Associator Growth (c074_associator_growth_empirical.csv) ---");
    ...
    for line in reader_alg.lines() {
        let l = line?;
        if l.starts_with('#') || l.is_empty() || l.starts_with("dimension") { continue; }
        ...
        if let (Ok(dim), Ok(norm_sq)) = (parts[0].parse::<f64>(), parts[1].parse::<f64>()) {
            ...
            println!("  Dim (g): {:.0} | Associator sq-norm (O): {:.4}", dim, norm_sq);
        }
    }
    ...
    if alg_points.len() >= 2 {
        ...
        for i in 0..(alg_points.len() - 1) {
            if let Ok(jac) = CouplerJacobian::estimate_from_delta(&alg_points[i], &alg_points[i+1]) {
                println!("    g range [{:.0}, {:.0}]: J = {:.4}", 
                         alg_points[i].g[0], alg_points[i+1].g[0], jac.j_mat[(0,0)]);
            }
        }
    }
    ...
    for line in reader_inf.lines().skip(1) {
        ...
        if let (Ok(n_attr), Ok(um_frac)) = (parts[2].parse::<f64>(), parts[5].parse::<f64>()) {
            ...
            println!("  Attributes (g): {:.0} | Ultrametric Fraction (O): {:.4} | Label: {}", 
                     n_attr, um_frac, parts[0]);
        }
    }

    // Filter for FRB points specifically to check scaling across attributes
    let frb_points: Vec<_> = inf_points.iter()
        .zip(vec!["FRB_3attr", "Pulsar_3attr", "Combined_3attr", "FRB_DM_only"])
    ...
    if frb_points.len() >= 2 {
        ...
        for i in 0..(sorted_frb.len() - 1) {
            if (sorted_frb[i+1].g[0] - sorted_frb[i].g[0]).abs() > 0.5 {
                if let Ok(jac) = CouplerJacobian::estimate_from_delta(&sorted_frb[i], &sorted_frb[i+1]) {
                    println!("    g range [{:.0}, {:.0}]: J = {:.4}", 
                             sorted_frb[i].g[0], sorted_frb[i+1].g[0], jac.j_mat[(0,0)]);
                }
            }
        }
    }
    ...
    println!("Information Jacobian (Data Cube Expansion): {:.4}", j_inf_mid);
    
    let delta = (j_alg_mid - j_inf_mid).abs();
    println!("Delta J: {:.4}", delta);
    
    if delta < 0.2 {
        println!("!! DISCOVERY: Deep alignment between Algebraic and Informational scaling detected !!");
        println!("The way non-associativity saturates in higher algebras mirrors the way hierarchical information ");
        println!("saturates in high-dimensional data cubes. They inhabit the same sector of the Coupler Manifold.");
    } else {
