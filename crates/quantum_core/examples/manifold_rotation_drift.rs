...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if points.len() >= 2 {
        ...
        println!(
            "but as the angle scale increases past 0.2, the Jacobian systematically decreases (J < 1.0). "
        );
        println!(
            "This indicates a 'compactification' effect where the non-associative geometry begins to fold back on itself, bounded by the finite volume of the Cayley-Dickson manifold."
        );
    }
