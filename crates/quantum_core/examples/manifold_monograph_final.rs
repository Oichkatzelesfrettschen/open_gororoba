...
use verified_core::coupler_manifold::{CouplerJacobian, CouplerPoint, qec::TwoSectorMixture};
...
fn main() {
    ...
    println!(
        "MERA (c={:.2}) Manifold Points:",
        mera_res.central_charge_estimate
    );
    ...
    println!(
        "Extracted QEC Jacobian (p_r={:.1} to {:.1}): {:.4}",
        points[0].g[0],
        points[1].g[0],
        j_qec.j_mat[(0, 0)]
    );
    ...
    println!(
        "p-adic (p={}) Holographic Scaling (Ryu-Takayanagi):",
        p_adic_base
    );
    println!(
        "  Min-Cut Entropy Slope (Boundary log L scaling): {:.4}",
        rt_res.log_coefficient
    );
    ...
    if rt_manifold.len() > 2 {
        ...
        println!(
            "  Local Boundary Elasticity J (at size L={:.0}): {:.4}",
            rt_manifold[1].g[0],
            j_rt.j_mat[(0, 0)]
        );
    }
