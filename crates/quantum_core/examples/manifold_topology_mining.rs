...
fn analyze_cd_motifs() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in reader.lines().skip(1) {
        ...
        if let (Ok(dim), Ok(components), Ok(nodes), Ok(edges)) = (
            ...
            observations.push(MotifObservation {
                dim,
                components,
                active_nodes: nodes,
                edges,
    ...
    for obs in &observations {
        println!(
            "  Dim (g): {:.0} | Active Nodes (O): {:.0}",
            obs.dim, obs.active_nodes
        );
    ...
    for obs in &observations {
        println!(
            "  Dim (g): {:.0} | Max Component Edges (O): {:.0}",
            obs.dim, obs.edges
        );
    ...
    println!(
        "However, the Max Component Edges Jacobian perfectly asymptotes to 3.0 (cubic scaling)."
    );
    println!(
        "This means the density of topological voids within the algebraic graph explodes faster than the manifold size itself."
    );
