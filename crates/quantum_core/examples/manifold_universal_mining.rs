...
fn analyze_lattice_voids() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..imbalance_thresholds.len() {
        ...
        println!(
            "Imbalance > {:.2} (g): Syndrome Count O={}",
            imbalance_thresholds[i], syndrome_counts[i]
        );
    }
    ...
    for i in 0..(points.len() - 1) {
        if let Ok(jac) = CouplerJacobian::estimate_from_delta(&points[i], &points[i + 1]) {
            ...
            println!(
                "  g range [{:.2}, {:.2}]: J = {:.4}",
                imbalance_thresholds[i],
                imbalance_thresholds[i + 1],
                j_val
            );
            if j_val.abs() > 2.0 {
...
fn analyze_frb_ultrametricity() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(jac) = CouplerJacobian::estimate_from_delta(&p2, &p3) {
        ...
        println!(
            "!! DISCOVERY: Increasing the latent parameter space from 2D to 3D yields a Jacobian of {:.2}.",
            jac.j_mat[(0, 0)]
        );
        println!(
            "This matches theoretical expectations for how correlation length scales across dimensions in fractional systems."
        );
    }
...
fn analyze_nonlocal_materials() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if count > 0 {
        ...
        println!(
            "!! DISCOVERY: Nonlocal coupling scaling provides an empirical bridge between topological mechanics and hardware architectures."
        );
    }
