...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let temperatures: Vec<f64> = vec![
        0.0, 0.001, 0.01, 0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1e6, 1e9, 1e12,
    ];
    ...
    for &dim in &dimensions {
        ...
        for &t in &temperatures {
            ...
            if (shift - 0.375).abs() < 1e-6 {
                println!(
                    "  Temp: {:9.3e} | K: {:10.2} | Phi: {:.6} | Shift: {:.6} (Total Melt)",
                    t, k, phi_opt, shift
                );
            } else if shift > 0.0001 {
                println!(
                    "  Temp: {:9.3e} | K: {:10.2} | Phi: {:.6} | Shift: {:.6} (Unlocking Flow)",
                    t, k, phi_opt, shift
                );
            } else {
                println!(
                    "  Temp: {:9.3e} | K: {:10.2} | Phi: {:.6} | Shift: {:.6} [Rigid / Bound]",
                    t, k, phi_opt, shift
                );
            }
