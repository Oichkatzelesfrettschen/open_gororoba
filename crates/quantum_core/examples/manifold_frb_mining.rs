...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, p) in points.iter().enumerate() {
        println!(
            "Bin {}: Hurst (g) = {:.3}, Ultrametricity (O) = {:.3}",
            i, p.g[0], p.o[0]
        );
    }
    ...
    if points.len() >= 4 {
        ...
        let j_persistent = CouplerJacobian::estimate_from_delta(
            &points[points.len() - 2],
            &points[points.len() - 1],
        )
        .unwrap();
        ...
        println!(
            "Brownian Sector (Low Hurst) Jacobian J: {:.3}",
            j_brownian.j_mat[(0, 0)]
        );
        println!(
            "Persistent Sector (High Hurst) Jacobian J: {:.3}",
            j_persistent.j_mat[(0, 0)]
        );
        ...
        if delta_j > 1.0 {
            println!(
                "CONCLUSION: Strong multi-sector behavior. The hierarchy of time-cascades fundamentally shifts the scaling manifold."
            );
        } else {
            println!(
                "CONCLUSION: Uniform scaling behavior across all temporal clustering regimes."
            );
        }
    }
