...
use quantum_core::tight_binding::{
    TightBindingModel, BravaisLattice2D, Vec2, OrbitalSite, Hopping, valley_chern_number, Valley,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let orbitals = vec![
        OrbitalSite { position: Vec2::new(0.0, 0.0), label: "A".to_string(), on_site_energy: 0.0 },
        OrbitalSite { position: Vec2::new(0.5, 0.0), label: "B".to_string(), on_site_energy: 0.0 },
        OrbitalSite { position: Vec2::new(0.25, 3.0f64.sqrt() / 4.0), label: "C".to_string(), on_site_energy: 0.0 },
    ];
    ...
    let hoppings = vec![
        ...
        Hopping { from: 0, to: 1, cell_offset: [0, 0], amplitude: c64::new(t, 0.0) },
        Hopping { from: 1, to: 2, cell_offset: [0, 0], amplitude: c64::new(t, 0.0) },
        Hopping { from: 2, to: 0, cell_offset: [0, 0], amplitude: c64::new(t, 0.0) },
        
        // Inter-cell (forming the continuous crystal)
        Hopping { from: 1, to: 0, cell_offset: [1, 0], amplitude: c64::new(t, 0.0) },
        Hopping { from: 2, to: 1, cell_offset: [-1, 1], amplitude: c64::new(t, 0.0) },
        Hopping { from: 0, to: 2, cell_offset: [0, -1], amplitude: c64::new(t, 0.0) },
    ];
    ...
    for band in 0..3 {
        ...
        println!("Band {}: VCN(K) = {:>6.3}, VCN(K') = {:>6.3}, Total Chern = {:>6.3}", 
                 band, vcn_k, vcn_kp, total_chern);
                 
        writeln!(file, "TRS_Motif,{},{:.4},{:.4},{:.4}", band, vcn_k, vcn_kp, total_chern)?;
    }
    ...
    for band in 0..3 {
        ...
        let total_chern = quantum_core::tight_binding::band_chern_number(&model_broken_trs, band, n_grid);
        
        println!("Band {}: VCN(K) = {:>6.3}, VCN(K') = {:>6.3}, Total Chern = {:>6.3}", 
                 band, vcn_k, vcn_kp, total_chern as f64);
                 
        writeln!(file, "Broken_TRS,{},{:.4},{:.4},{:.4}", band, vcn_k, vcn_kp, total_chern as f64)?;
    }

    println!("\n✅ Magnonic exact analysis mapped to {}", out_path.display());
