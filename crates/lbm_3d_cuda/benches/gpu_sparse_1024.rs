...
fn main() {
    ...
    println!(
        "Domain Size: {}^3 ({} billion cells)",
        nx,
        total_cells as f64 / 1e9
    );
    ...
    let mut d_mask = stream
        .alloc_zeros::<u8>(total_cells)
        .expect("Failed to allocate 1GB mask");
    ...
    let map = SparseBrickMap::new_from_geometry(ctx.clone(), stream.clone(), nx, ny, nz, &d_mask)
        .unwrap();
    ...
    println!(
        "  Throughput:   {:.2} MLUPS (Active Cells Only)",
        throughput
    );
    println!(
        "  Effective:    {:.2} GLUPS (Dense Equivalent)",
        (total_cells as f64 * steps as f64) / evolve_time.as_secs_f64() / 1e9
    );
    println!("{}", "#".repeat(80));
}
