//! D3Q19 lattice Boltzmann method with MRT collision, Vulkan compute backend.
//!
//! Uses the consolidated `gororoba_gpu_vulkan` helpers and the MRT shader at
//! `shaders/lbm_mrt_d3q19.wgsl`. The collision operator follows d'Humieres
//! (2002) with five relaxation rates: s_nu = 1/tau (stress moments), S_E =
//! 1.19 (energy), S_EPS = 1.40 (energy squared), S_Q = 1.20 (energy flux),
//! S_GHOST = 1.00 (ghost moments).
//!
//! Buffer layout, PUSH scheme, and ping-pong semantics are identical to the
//! BGK variant (`lbm_d3q19_vulkan.rs`).
//!
//! Public API: [`LbmMrtD3Q19Vulkan::new`] constructs everything;
//! [`LbmMrtD3Q19Vulkan::evolve`] runs N steps;
//! [`LbmMrtD3Q19Vulkan::compute_macroscopic`] reads back (rho, ux, uy, uz).
...
const WGSL_SOURCE: &str = include_str!("../shaders/lbm_mrt_d3q19.wgsl");
...
/// Errors emitted by the D3Q19 MRT Vulkan launcher.
#[derive(Debug, thiserror::Error)]
pub enum LbmMrtD3Q19Error {
...
impl From<vk::Result> for LbmMrtD3Q19Error {
...
/// Vulkan-backed D3Q19 MRT LBM solver.
pub struct LbmMrtD3Q19Vulkan {
    nx: usize,
    ny: usize,
    nz: usize,
    n_cells: usize,
...
impl LbmMrtD3Q19Vulkan {
    /// Build a fresh MRT solver for an `nx*ny*nz` periodic D3Q19 grid with
    /// relaxation time `tau` (must satisfy tau > 0.5 for MRT stability).
    pub fn new(nx: usize, ny: usize, nz: usize, tau: f32) -> Result<Self, LbmMrtD3Q19Error> {
        if nx == 0 || ny == 0 || nz == 0 {
            return Err(LbmMrtD3Q19Error::EmptyGrid { nx, ny, nz });
        }
        ...
        if n_cells_u64 > u32::MAX as u64 {
            return Err(LbmMrtD3Q19Error::GridTooLarge(n_cells_u64));
        }
        if tau.is_nan() || tau <= 0.5 {
            return Err(LbmMrtD3Q19Error::UnstableTau(tau));
        }
        let n_cells = nx * ny * nz;

        let instance = InstanceBuilder::new("lbm_mrt_d3q19_vulkan")
    ...
    /// Replace the current f field with the host-supplied SoA slice
    /// (`f[i * n_cells + cell]`, `i = 0..19`).
    pub fn upload_state(&mut self, f_host: &[f32]) -> Result<(), LbmMrtD3Q19Error> {
        let expected = self.n_cells * D3Q19_CHANNELS;
        if f_host.len() != expected {
            return Err(LbmMrtD3Q19Error::UploadLengthMismatch {
                got: f_host.len(),
                expected,
            });
        }
    ...
    /// Advance the simulation by `num_steps` MRT + stream timesteps.
    pub fn evolve(&mut self, num_steps: usize) -> Result<(), LbmMrtD3Q19Error> {
    ...
    /// Read the current f field back to host memory (SoA layout).
    pub fn download(&self) -> Result<Vec<f32>, LbmMrtD3Q19Error> {
        download_f32_slice(&self.device, &self.f_buffers[self.current_in])
    }

    /// Compute (rho, ux, uy, uz) per cell from the current f field.
    pub fn compute_macroscopic(&self) -> Result<MacroFields, LbmMrtD3Q19Error> {
        let f = self.download()?;
        let n = self.n_cells;
...
impl Drop for LbmMrtD3Q19Vulkan {
    fn drop(&mut self) {
        // SAFETY: descriptor_pool was created on self.device above and
        // is destroyed here exactly once.
        unsafe {
            self.device
                .raw()
...
fn allocate_storage_buffer(
    ...
    instance: &Instance,
    size: vk::DeviceSize,
) -> Result<DeviceBuffer, LbmMrtD3Q19Error> {
    allocate_buffer(
        device,
        adapter,
        instance,
...
fn allocate_buffer(
    ...
    // SAFETY: device + adapter belong to the same instance; size is bounded
    // by the calling solver (n_cells * 19 * 4 bytes).
    let buffer = unsafe {
    ...
    if mem_type_index == u32::MAX {
        ...
        return Err(LbmMrtD3Q19Error::Vk(vk::Result::ERROR_OUT_OF_DEVICE_MEMORY));
    }
    // SAFETY: req.size + mem_type_index are produced by Vulkan.
    let memory = unsafe {
