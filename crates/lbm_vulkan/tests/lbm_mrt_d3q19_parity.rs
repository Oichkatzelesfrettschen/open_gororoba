//! 3-way parity test: CPU `lbm_3d::LbmSolver3D` (f64 MRT) vs Vulkan
//! `LbmMrtD3Q19Vulkan` (f32 WGSL MRT) vs cubecl-wgpu
//! `evolve_mrt_d3q19_cubecl` (f32 cubecl MRT).
//!
...
//! uy, uz) within abs_tol=2e-3 / rel_tol=2e-2.
//!
...
//! individually. If both skip the test returns early -- with neither GPU path
//! live there is nothing to compare and the 3-way contract is unverifiable.
//!
//! Run with:
//!   cargo test -p lbm_vulkan --features cubecl --release \
//!     lbm_mrt_d3q19_parity -- --ignored --nocapture
...
// Seed distinct from the two per-backend MRT seeds.
const SEED: u64 = 0xD319_4D52_5400_3A1F;

#[test]
#[ignore = "gpu (Vulkan ICD + cubecl-wgpu adapter required)"]
fn three_way_mrt_parity_16cubed_10steps() {
    ...
    let vulkan_result: Option<Vec<(f64, [f64; 3])>> =
        match LbmMrtD3Q19Vulkan::new(NX, NY, NZ, TAU as f32) {
            Err(e) => {
    ...
    let cubecl_result: Option<Vec<(f64, [f64; 3])>> = if !is_available() {
        ...
        match evolve_mrt_d3q19_cubecl(NX, NY, NZ, TAU as f32, &f_init, STEPS) {
            Err(e) => {
    ...
    if vulkan_result.is_none() && cubecl_result.is_none() {
        eprintln!(
            "skip: neither Vulkan nor cubecl backend available; \
             3-way MRT parity test skipped"
        );
        return;
    }
    ...
    if vulkan_result.is_some() {
        eprintln!(
            "CPU-vs-Vulkan-MRT OK on {}x{}x{} after {} steps: \
    ...
    if cubecl_result.is_some() {
        eprintln!(
            "CPU-vs-cubecl-MRT OK on {}x{}x{} after {} steps: \
