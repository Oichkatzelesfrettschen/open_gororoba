...
// Cli + Args + DEFAULT_* + Table type alias live in the `types`
// submodule (~265 lines). #[path] needed because of explicit
// Cargo.toml path.
#[path = "csv_canonicalization/types.rs"]
mod types;
use types::*;
...
fn run_inventory(repo_root: &Path, args: &InventoryArgs) -> Result<()> {
    ...
    for rel in files {
        ...
        let (action, priority, rationale) = policy_with_progress(
            &rel,
            &zone,
            &CsvCanonicalProgress {
                legacy_canonical_paths: &legacy_canonical_paths,
                curated_canonical_paths: &curated_canonical_paths,
                project_canonical_paths: &project_canonical_paths,
                project_generated_paths: &project_generated_paths,
                external_holding_paths: &external_holding_paths,
                archive_holding_paths: &archive_holding_paths,
                external_holding_scroll_paths: &external_holding_scroll_paths,
                archive_holding_scroll_paths: &archive_holding_scroll_paths,
                project_split_classification: &project_split_classification,
            },
        );
...
fn run_migration_scope(repo_root: &Path, args: &MigrationScopeArgs) -> Result<()> {
    ...
    write_ascii(
        &out_path,
        &(render_migration_scope(MigrationScopeInputs {
            inventory_path: &args.inventory.to_string_lossy(),
            document_count: docs.len(),
            zone_counts: &zone_counts,
            action_counts: &action_counts,
            priority_counts: &priority_counts,
            high_priority_paths: &high_priority_paths,
            medium_priority_paths: &medium_priority_paths,
            next_waves: &[
                (
                    "wave_1",
                    wave_state(
...
/// Bundle of migration-scope render inputs. Three count maps (zone /
...
struct MigrationScopeInputs<'a> {
    ...
    zone_counts: &'a BTreeMap<String, usize>,
    action_counts: &'a BTreeMap<String, usize>,
    priority_counts: &'a BTreeMap<String, usize>,
    high_priority_paths: &'a [String],
    medium_priority_paths: &'a [String],
    next_waves: &'a [(&'a str, String)],
}

fn render_migration_scope(inputs: MigrationScopeInputs<'_>) -> Result<String> {
    let inventory_path = inputs.inventory_path;
    let document_count = inputs.document_count;
    let zone_counts = inputs.zone_counts;
    let action_counts = inputs.action_counts;
    let priority_counts = inputs.priority_counts;
    let high_priority_paths = inputs.high_priority_paths;
    let medium_priority_paths = inputs.medium_priority_paths;
    let next_waves = inputs.next_waves;
    let mut lines = vec![
...
/// Bundle of CSV canonicalization progress sets used by
...
struct CsvCanonicalProgress<'a> {
    ...
    curated_canonical_paths: &'a BTreeSet<String>,
    project_canonical_paths: &'a BTreeSet<String>,
    project_generated_paths: &'a BTreeSet<String>,
    external_holding_paths: &'a BTreeSet<String>,
    archive_holding_paths: &'a BTreeSet<String>,
    external_holding_scroll_paths: &'a BTreeSet<String>,
    archive_holding_scroll_paths: &'a BTreeSet<String>,
    project_split_classification: &'a BTreeMap<String, String>,
}

fn policy_with_progress(
    path: &str,
    zone: &str,
    progress: &CsvCanonicalProgress<'_>,
) -> (String, String, String) {
    let legacy_canonical_paths = progress.legacy_canonical_paths;
    let curated_canonical_paths = progress.curated_canonical_paths;
    let project_canonical_paths = progress.project_canonical_paths;
    let project_generated_paths = progress.project_generated_paths;
    let external_holding_paths = progress.external_holding_paths;
    let archive_holding_paths = progress.archive_holding_paths;
    let external_holding_scroll_paths = progress.external_holding_scroll_paths;
    let archive_holding_scroll_paths = progress.archive_holding_scroll_paths;
    let project_split_classification = progress.project_split_classification;
