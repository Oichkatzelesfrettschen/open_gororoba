...
fn verify_semantic_atoms(repo_root: &Path, args: &Args) -> Result<()> {
    let required = [
        ...
        repo_root.join(&args.symbol_table_out),
        repo_root.join(&args.proof_skeletons_out),
        repo_root.join(&args.payload_path),
        repo_root.join(&args.payload_chunks_path),
    ];
    ...
    let payload_raw = load_toml(&repo_root.join(&args.payload_path))?;
    let chunk_raw = load_toml(&repo_root.join(&args.payload_chunks_path))?;
