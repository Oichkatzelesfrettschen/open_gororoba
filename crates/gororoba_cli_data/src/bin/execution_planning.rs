...
fn build_execution_planning(repo_root: &Path, args: &Args) -> Result<()> {
    ...
    write_ascii(
        ...
        &(render_roadmap(&roadmap_meta, &roadmap_rows)? + "\n"),
    )?;
    write_ascii(
        &repo_root.join(&args.todo_out),
        &(render_todo(&todo_rows)? + "\n"),
    )?;
    write_ascii(
        &repo_root.join(&args.next_actions_out),
        &(render_next_actions(&action_rows)? + "\n"),
    )?;
    write_ascii(
        &repo_root.join(&args.requirements_out),
        &(render_requirements(&req_modules, &req_gaps, &req_meta)? + "\n"),
    )?;
    write_ascii(
...
fn render_roadmap(meta: &Table, rows: &[PlanningRow]) -> Result<String> {
    ...
    let mut lines = vec![
        "# Operational roadmap registry (schema-hardened TOML lane pending SQLite promotion).".to_string(),
        "# Generated by src/scripts/analysis/build_wave5_batch4_registries.py.".to_string(),
        String::new(),
        "[roadmap]".to_string(),
        format!(
...
fn render_todo(rows: &[PlanningRow]) -> Result<String> {
    let mut lines = vec![
        "# To-Do Registry (schema-hardened TOML lane pending SQLite promotion).".to_string(),
        "# Generated by src/scripts/analysis/build_wave5_batch4_registries.py.".to_string(),
        String::new(),
        "[todo]".to_string(),
        "updated = \"2026-02-10\"".to_string(),
...
fn render_next_actions(rows: &[PlanningRow]) -> Result<String> {
    let mut lines = vec![
        "# Next Actions Registry (schema-hardened TOML lane pending SQLite promotion).".to_string(),
        "# Generated by src/scripts/analysis/build_wave5_batch4_registries.py.".to_string(),
        String::new(),
        "[meta]".to_string(),
        "updated = \"2026-02-10\"".to_string(),
...
fn render_requirements(
    ...
    let mut lines = vec![
        "# Requirements registry (schema-hardened TOML lane pending SQLite promotion).".to_string(),
        "# Generated by src/scripts/analysis/build_wave5_batch4_registries.py.".to_string(),
        String::new(),
