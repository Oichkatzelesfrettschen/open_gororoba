...
// Clap Cli/Commands enum + all *Args structs + Svg* deserialize
// types live in the `args` submodule. ~1000 lines of declarative
// type definitions split out so this bin's dispatch + emitter logic
// is navigable. Uses `#[path]` because this binary has an explicit
// `path = ...` in Cargo.toml which disables Rust's implicit
// foo.rs/foo/<sibling>.rs sibling-file lookup.
#[path = "registry_emit/args.rs"]
mod args;
use args::*;
