...
//! Sedenion field frustration.

use crate::bridge::SedenionField;
use optics_core::grin::{central_difference_gradient, GrinMedium, Vec3};

/// A refractive medium driven by algebraic vacuum frustration.
pub struct AlgebraicLensingMedium {
    /// Precomputed frustration density field (flattened 3D grid)
...
impl AlgebraicLensingMedium {
    ...
    /// Precomputes the frustration density map (expensive) once.
    pub fn new(field: &SedenionField, dim: usize, alpha: f64) -> Self {
        // Compute the full density field upfront
        let density_field = field.local_frustration_density(dim);
    ...
    /// n(p) = n0 + alpha * frustration_density(p)
        ...
        for z in 0..8 {
            for y in 0..8 {
                for x in 0..8 {
                    ...
                    if dist < 3.0 {
                        // High frustration core: mix basis elements
                        s[1] = 1.0;
                        s[2] = 1.0; 
                        s[3] = 1.0;
                    }
