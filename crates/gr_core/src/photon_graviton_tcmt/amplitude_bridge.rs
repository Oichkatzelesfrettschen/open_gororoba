...
/// * `graviton_coupling_strength` - α_grav or similar dimensionless coupling
///
/// # Returns
/// Coupling strength κ for TCMT (units: frequency)
pub fn extract_coupling_from_irreducible(
    irreducible_amplitude: f64,
    photon_frequency: f64,
    graviton_coupling_strength: f64,
) -> f64 {
    // In weak-field limit: κ ~ M_irreducible * α_grav * ω_photon
...
///
/// # Returns
/// Radiative decay rate γ_rad (units: frequency)
pub fn extract_decay_from_tadpole(
    _tadpole_real_part: f64,
    tadpole_imag_part: f64,
) -> f64 {
...
/// This maps to the Fano asymmetry parameter phase φ.
///
/// # Arguments
/// * `external_leg_amplitude` - Complex amplitude M_external from E-073
///
/// # Returns
/// Phase φ in radians, range [0, 2π]
...
///
/// # Returns
/// true if weak-field parameter ε < 0.1
...
/// κ₁ - κ₂ = γ_l (energy conservation constraint)
///
/// In our photon-graviton system:
/// κ_ph - κ_grav = γ_coupling
///
/// # Arguments
...
/// * `graviton_coupling` - Gravitational coupling constant α_grav
///
/// # Returns
...
pub fn three_diagram_amplitude_to_tcmt(
    ...
    let coupling_strength = extract_coupling_from_irreducible(irreducible, photon_frequency, graviton_coupling);
...
mod tests {
    ...
    fn weak_field_from_three_diagrams() {
        let coupling = three_diagram_amplitude_to_tcmt(
            1e-10, 0.1, 1e-6, 0.5, 1e15, 1e-45,
        );
