...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Fluid Jacobian J ({:.2} -> {:.2}): {:.4}", thresholds[0], thresholds[1], j_fluid.j_mat[(0,0)]);
    ...
    if let Ok(jac_smooth) = CouplerJacobian::estimate_from_delta(&p_smooth_0, &p_smooth_1) {
        println!("Floor-Subtracted Fluid Jacobian J: {:.4}", jac_smooth.j_mat[(0,0)]);
    ...
    if j_cross.is_empty() {
        println!("!! NOVEL INSIGHT: The fluid defect scaling is statistically consistent with the pure algebraic phase transition !!");
        println!("This suggests the 'Defect Floor' in the fluid is a direct projection of the Sedenion non-associativity bound.");
    } else {
        println!("Mismatched Scaling: Delta J = {:.2}", (j_alg.j_mat[(0,0)] - j_fluid.j_mat[(0,0)]).abs());
        println!("The fluid adds extra complexity beyond the static algebra (likely due to dynamical dissipation).");
    }
