//! Tests for boxkites. Relocated from inline
//! `#[cfg(test)] mod tests { ... }` to a sibling file via
//! `mod tests;` in the parent so the parent module stays focused
//! on production code (633 lines vs 7665 with tests inline).
...
fn test_motif_census_16d_matches_de_marrais_box_kites() {
    ...
    let union: HashSet<CrossPair> = comps.iter().flat_map(|c| c.nodes.iter().copied()).collect();
...
fn test_separator_generalization() {
    ...
    let antidet =
        |dim: usize, a: CrossPair, b: CrossPair| -> u8 { psi(dim, a.0, b.1) ^ psi(dim, a.1, b.0) };

    // The "diagonal XOR" = sigma exponent per edge (Same iff 1)
    let diag =
        |dim: usize, a: CrossPair, b: CrossPair| -> u8 { psi(dim, a.0, b.0) ^ psi(dim, a.1, b.1) };
