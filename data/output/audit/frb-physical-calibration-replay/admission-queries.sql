ATTACH '.cache/frb-calibration-admission/before.sqlite3' AS baseline;
...
UNION ALL SELECT 'changed_other_insights',count(*) FROM (SELECT * FROM baseline.insights WHERE id NOT IN ('I-001','I-002') EXCEPT SELECT * FROM main.insights)
UNION ALL SELECT 'changed_other_insight_evidence',count(*) FROM (SELECT * FROM baseline.insight_evidence WHERE insight_id NOT IN ('I-001','I-002') EXCEPT SELECT * FROM main.insight_evidence)
