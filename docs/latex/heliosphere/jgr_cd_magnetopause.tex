...
%%
%% To compile:
%%   latexmk -pdf -shell-escape -output-directory=out jgr_cd_magnetopause.tex
%%
%% For AGU submission: replace \documentclass{article} below with
%%   \documentclass[jgrspacephysics,manuscript]{agujournal2019}
%% and obtain agujournal2019.cls from the AGU author-resources page.
%%
...
An Alfvenic control experiment on Parker Solar Probe Encounter 3
...
The Takens embedding theorem \citep{takens1981} provides a principled way to
...
\begin{enumerate}
  \item THEMIS-A dayside magnetopause (Earth-proximal, Section~\ref{sec:tau_sweep}),
  \item Parker Solar Probe inner heliosphere (Alfvenic vs.\ compressive intervals,
    Section~\ref{sec:alfven_control}),
  \item Parker Solar Probe micro-switchback correlation (Section~\ref{sec:enrichment}),
  \item MMS fluorescence topology (extra-detection classification,
...
via the Cayley--Dickson doubling process \citep{dickson1919}: starting from
...
\begin{equation}
  ...
  \label{eq:associator}
\end{equation}
A detection is fired when $\Assoc(t)$ exceeds an adaptive threshold based on
the local 15-minute sliding median-absolute-deviation.
...
\end{equation}
where $\bar{B}$ is the mean $|B|$ over the window and $\eps_0 = 0.1$~nT is a
noise-floor constant.  When $|B| < \eps_0$, the denominator locks to $\eps_0$,
...
Staples et al.\ \citeyearpar{staples2020} THEMIS-A magnetopause crossing catalog.
The evaluation interval is 2016-08-29 to 2016-09-05 (7 days, $n = 7{,}968$
FGM minutes), containing 224 labeled crossing events.  A detection within
$\pm 5$~min of a catalog event counts as a true positive.
...
%% ===================================================================
%% 4. ALFVENIC CONTROL ANALYSIS (PSP E3)
%% ===================================================================
...
1-minute interval in Parker Solar Probe Encounter 3
(2020-01-20 to 2020-02-10, $n = 31{,}673$ minutes) into one of three classes
...
\begin{figure}[htbp]
  ...
  \caption{CD associator and gradient fire rates by window class for PSP
    Encounter 3 (2020-01-20 -- 2020-02-10, 31{,}673 minutes).
...
\begin{table}[htbp]
  \centering
  \caption{PSP E3 fire rates by window class (2020-01-20 to 2020-02-10).}
...
%% ===================================================================
%% 5. MICRO-SWITCHBACK CORRELATION (PSP E3)
%% ===================================================================
...
\begin{table}[htbp]
  \centering
  \caption{PSP E3 micro-switchback enrichment (quiet-interval CD fires only).}
...
The $1.72\times$ enrichment confirms that quiet CD fires are preferentially
associated with kinetic-scale topology changes below the Alfvenic classifier
threshold.  However, the $32.5\%$ rotation-free population is physically
significant: these 39 fires coincide with no detectable B-vector rotation in
the $\pm 3$-minute neighborhood.

We interpret this population as intra-minute vector cancellation events where
the B-vector undergoes a full rotation within a single 60-second sample
window, producing a ``magnitude pinch'' (reduction of $|B|$ without a
detectable rotation in the 1-minute averages) that is visible in
$\mathbb{R}^{32}$ through the delay embedding but invisible to the
3-dimensional rotation metric.  The 32-dimensional associator is sensitive
to these sub-minute topological events because the delay vector encodes
eight consecutive field states simultaneously; the 3-dimensional rotation
metric sees only the averaged result.
...
We apply the CD associator to MMS-1 data for 2024-01-01 to 2024-01-07
(seven days crossing the dayside magnetopause) and compare detections to the
MMS Fast Plasma Investigation (FPI) plasma-boundary catalog \citep{pollock2016}.
...
positive fraction is consistent with the interpretation that CD fires in
quiet plasma identify flux-transfer event (FTE) structures that are not
captured in kinetic-scale FPI catalogs due to plasma-signature requirements.
...
in PSP E3 are the clearest evidence: after the Alfvenic classifier has removed
...
This population can only arise from intra-minute structure: field rotations
that complete within one 60-second bin, leaving no trace in 1-minute averages
but distorting the delay vector through non-commutativity of the
trigintaduonion product.
...
\begin{itemize}
  ...
  \item \emph{CD associator}: highest F1 on single-spacecraft survey data;
    topologically selective; sensitive to sub-minute structure.
\end{itemize}
...
\begin{enumerate}
  ...
  \item PSP E3 Alfvenic control analysis yields a $2.83\times$
  ...
  \item Quiet-interval CD fires on PSP E3 are $1.72\times$ enriched near
    micro-rotation events ($\geq 15^\circ/\mathrm{min}$), but $32.5\%$
    occur with no nearby rotation event -- indicating sensitivity to
    sub-minute topology changes invisible to 3-dimensional rotation measures.
...
THEMIS FGM data were obtained from the HAPI server
...
(\url{https://spdf.gsfc.nasa.gov/}) as product \texttt{PSP\_FLD\_L2\_MAG\_RTN\_1MIN}.
All analysis code is available in the open\_gororoba repository
under the experiments E-237, E-239, E-240, and E-241;
see \texttt{registry/experiments.toml} for detailed provenance.
