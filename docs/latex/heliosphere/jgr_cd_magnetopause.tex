...
%%   experiments E-267..E-271; claim registration pending
...
The $32$-dimensional Cayley--Dickson algebra (the \emph{trigintaduonions}) provides
a canonical trilinear alternator for this embedding space: the \emph{associator}
$[x,y,z] \equiv (x \cdot y) \cdot z - x \cdot (y \cdot z)$.
...
The dimensional ablation in Section~\ref{sec:dimensional_ablation} establishes
the empirical performance of the CD associator relative to random trilinear
alternatives and motivates the framing of CD as a canonical member of a
broader nonlinear trilinear family, rather than as the uniquely optimal choice.
...
\begin{equation}
  ...
  \label{eq:associator}
\end{equation}
A detection is fired when the local jump in $\Assoc(t)$ exceeds a fixed
threshold equal to \texttt{MAD\_SCALE\_FACTOR}$= 1.5$ times the global standard
deviation of $\Assoc$ computed over the full evaluation window.
This scale factor was fixed prior to any evaluation on labeled test data.

\paragraph{Threshold disclosure.}
The global-standard-deviation threshold integrates information from the
full evaluation window, introducing a form of in-sample calibration.
To quantify this effect, we re-evaluated the same associator with a held-out
threshold: for each detection candidate, the threshold is estimated from the
preceding disjoint 15-minute window only.  Under this decorrelated estimator,
F1 falls from 0.601 to 0.332, indicating that approximately half of the
headline performance is attributable to window-integrated threshold calibration
rather than boundary discrimination alone.
Both values are reported in Table~\ref{tab:ablation}; the held-out F1 = 0.332
represents the conservative estimate for deployment on new data.
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_tau_sweep.tex}
...
The CD associator achieved $\mathrm{F1}=0.415$ (95\% CI $[0.320, 0.457]$,
null $p < 10^{-4}$) over 447 catalog events on the 30-day window, compared with
$\mathrm{F1}=0.601$ on the 7-day SC window.
The gradient-magnitude baseline achieved F1$=0.343$ on the same 30-day window.
The reduction in F1 from the SC window is expected: the 30-day interval includes
quiet stretches with no SC-associated transitions, which increase the effective
false-alarm rate and reduce event density relative to the SC-selected window.
The non-zero F1 and statistically significant null p-value confirm that the CD
associator identifies real structure even in unselected intervals, but the
SC-selection bias must be accounted for when interpreting the Section~3.1 headline.
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_alfven_control.tex}
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_enrichment.tex}
...
\begin{figure}[htbp]
  \centering
  \input{figures/fig_fte_scatter.tex}
...
$V_x > 100$~km/s post-crossing; \citet{eastwood2016}), using the FTE morphology
signatures established by \citet{russell1978}, is needed to
...
All five pre-registered predictions (P1--P5) were tested; only P4 was confirmed.
P1 (CD R32 $>$ L2-delay by $\geq 0.05$), P2 (CD R32 $>$ dense-random mean
by $\geq 2\sigma$), P3 (F1 monotonically decreasing from $d=8$ to $d=4$),
and P5 (CD R64 within 0.05 of CD R32) were all falsified.
P4 (CD R16 F1 $<$ CD R32 F1) was strongly confirmed.

\textbf{Pre-registered predictions falsified.}  The L2-delay baseline
(F1$=0.615$) exceeds CD R32 (F1$=0.601$), falsifying P1.  CD R32 falls within
1.30$\sigma$ of the dense-random mean and 1.57$\sigma$ of the sparsity-matched
mean, falsifying P2.  The shorter $d=4$ window (F1$=0.625$) outperforms the
$d=8$ paper baseline, falsifying P3.  CD R64 (F1$=0.419$) is $0.182$ below R32,
falsifying P5.  The single confirmed prediction, P4, shows that removing Bz
and $|B|$ reduces F1 by $0.394$, confirming those channels are critical.

\textbf{Linear baseline.}  PCA variance achieves F1$=0.127$, establishing that
linear variance concentration near-trivially fails to detect boundaries.
Nonlinear algebraic structure is necessary, though not uniquely realized by CD.

\textbf{Honest framing.}  CD R32 does not demonstrably outperform random trilinear
forms with the same sparsity pattern at the pre-registered 2$\sigma$ level.
The CD algebra is one canonical member of the nonlinear trilinear family; other
members (including random ones) achieve comparable or superior F1 on this window.
The held-out-threshold F1 is 0.332 (see Section~\ref{sec:method}), which is the
appropriate estimate for out-of-sample deployment.

\begin{table}[htbp]
\caption{Pre-registered dimensional ablation results (THEMIS-A, 2016-DOY-242--248,
commit \texttt{455d4745}).
All F1 values use detection-level precision, event-level recall, $\pm 10$~min match.
CI is 95\% block-bootstrap (1800~s blocks, $N=10{,}000$, seed~42).
``Held-out F1'' uses preceding-window threshold (Section~\ref{sec:method}).
$\sigma_\mathrm{dense}$ = sigma units above dense-random mean; $\sigma_\mathrm{sparse}$ = sigma units above sparsity-matched mean.}
\label{tab:ablation}
\begin{tabular}{lrrlrr}
\toprule
Method & Dim & F1 & F1 CI 95\% & $\sigma_\mathrm{dense}$ & $\Delta$ vs L2-delay \\
\midrule
L2-delay baseline           & 32 & 0.615 & -- & -- & -- \\
PCA variance                & 32 & 0.127 & [0.076, 0.226] & -- & $-0.488$ \\
Commutator $\|xy-yx\|$      & 32 & 0.590 & [0.452, 0.675] & -- & $-0.025$ \\
Dense-random (mean$\pm$std) & 32 & $0.528\pm0.056$ & -- & -- & $-0.087$ \\
Sparsity-matched (mean$\pm$std) & 32 & $0.535\pm0.042$ & -- & -- & $-0.080$ \\
CD R16 ($d=8$, 2ch)         & 16 & 0.207 & -- & -- & $-0.408$ \\
\textbf{CD R32 ($d=8$)}     & \textbf{32} & \textbf{0.601} & [0.456, 0.667] & $+1.30$ & $-0.014$ \\
CD R32 held-out threshold   & 32 & 0.332 & [0.265, 0.399] & -- & $-0.283$ \\
CD R64 ($d=8$, 8ch)         & 64 & 0.419 & -- & -- & $-0.196$ \\
CD $d=4$ (4ch)              & 16 & 0.625 & -- & -- & $+0.010$ \\
CD $d=16$ (4ch)             & 64 & 0.419 & -- & -- & $-0.196$ \\
\bottomrule
\end{tabular}
\end{table}
...
under the experiments E-267, E-269, E-270, and E-271;
...
%% ===================================================================
%% SUPPLEMENTAL: BONFERRONI MULTIPLE-COMPARISON TABLE
%% ===================================================================
\section*{Supplemental: Multiple-Comparison Summary (Bonferroni)}

The following table enumerates all primary hypothesis tests reported in this
paper.  Family-wise error rate is controlled at $\alpha_{\mathrm{FW}} = 0.05$
over $m=17$ tests, giving per-test threshold $\alpha^* = 0.05/17 = 0.0029$.
Descriptive statistics (fire rates, coverage fractions, CI bounds) and
within-ablation comparisons that were pre-registered as prediction verdicts
are listed separately and are not subject to Bonferroni correction.

\begin{table}[htbp]
\caption{Multiple-comparison summary.  $p < \alpha^*$ in the Survives column
indicates the finding survives Bonferroni correction.  $m=17$ primary tests;
$\alpha^* = 0.0029$.  Tests marked ``permutation'' used $N=10{,}000$ shuffles;
tests marked ``shuffle-event'' shuffled catalog event times.  Pre-registered
prediction verdicts (P1--P5) are shown as ablation comparisons, not as
independent hypothesis tests.}
\label{tab:bonferroni}
\begin{tabular}{llrrl}
\toprule
Test & Statistic & Obs.\ value & $p$-value & Survives? \\
\midrule
\multicolumn{5}{l}{\textit{Primary signal tests}} \\
THEMIS SC F1 null (E-274)       & F1 = 0.601 vs shuffle & 0.601 & $< 10^{-4}$ & YES \\
THEMIS general F1 null (E-275)  & F1 = 0.415 vs shuffle & 0.415 & $< 10^{-4}$ & YES \\
PSP ratio permutation (E-273)   & 2.83x count ratio     & 0.941 & $< 10^{-4}$ & YES \\
\midrule
\multicolumn{5}{l}{\textit{Ablation comparisons (pre-registered; not Bonferroni-adjusted)}} \\
P1: CD R32 vs L2-delay          & $\Delta$F1 = -0.014   & --    & -- & FALSIFIED \\
P2: CD R32 vs dense-random      & 1.30$\sigma$ above    & --    & -- & FALSIFIED \\
P2b: CD R32 vs sparsity-matched & 1.57$\sigma$ above    & --    & -- & FALSIFIED \\
P3: monotone lag d=8 to d=4     & d=4 better (0.625)    & --    & -- & FALSIFIED \\
P4: R16 < R32                   & $\Delta$F1 = -0.394   & --    & -- & CONFIRMED \\
P5: R64 within 0.05 of R32      & $\Delta$F1 = -0.182   & --    & -- & FALSIFIED \\
\midrule
\multicolumn{5}{l}{\textit{Descriptive statistics (no correction applied)}} \\
Tau sweep monotonicity (E-270)  & F1 vs tau             & --    & -- & DESC. \\
MMS topology-positive (E-268)   & 75/86 = 87.2\%        & --    & -- & DESC. \\
PSP micro-rot enrichment (E-271)& 1.72$\times$          & --    & -- & DESC. \\
SC-window bootstrap CI (E-272)  & CI = [0.456, 0.667]   & --    & -- & DESC. \\
General-window CI (E-275)       & CI = [0.320, 0.457]   & --    & -- & DESC. \\
Held-out threshold F1 (E-276)   & F1 = 0.332            & --    & -- & DESC. \\
V1 heliopause enrichment (E-277)& 1.44$\times$          & --    & -- & DESC. \\
PCA linear baseline F1          & F1 = 0.127            & --    & -- & DESC. \\
\bottomrule
\end{tabular}
\end{table}

Three hypothesis tests survive Bonferroni correction at $\alpha^* = 0.0029$,
all with $p < 10^{-4}$: the THEMIS SC-window F1 (E-274), the general-interval
F1 (E-275), and the PSP compressive/Alfvenic discrimination ratio (E-273).
The five ablation predictions are pre-registered and reported as prediction
verdicts, not as independent hypothesis tests subject to this correction.
Only P4 (R16 $<$ R32) was confirmed.
