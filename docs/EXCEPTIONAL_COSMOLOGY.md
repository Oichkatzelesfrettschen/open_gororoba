...
**G2** is the automorphism group of the octonions. Its 14-dimensional adjoint representation appears in the sedenion ZD geometry: the G2 × S3 automorphism group of the sedenion algebra (where S3 permutes box-kite orientation) is the bridge between exceptional algebras and the Cayley-Dickson tower.

### F4 and the Casimir Ratio

**Verified (C-035, Rocq 9.1):** The F4 second Casimir ratio is:

    ε = C₂(26) / |Δ₊(F4)| = 6/24 = 1/4 exactly.

This is a pure algebraic identity. It was proposed as structurally related to the Barbero–Immirzi parameter γ ≈ 0.2375 (loop quantum gravity) and the Gauss–Bonnet coupling constant 4λ_GB.
...
**Statement:** F4 Casimir ratio ε = C₂(26)/|Δ₊(F4)| = 6/24 = 1/4 exactly.
...
### C-037: Numerical Coincidence γ ~ ε ~ 4λ_GB (Refuted)

**Statement:** γ ≈ ε ≈ 4λ_GB ≈ 1/4, relating the Barbero–Immirzi parameter, network clustering, and Gauss–Bonnet coupling.
...
### C-038: Dark Energy Equation of State w₀ = −5/6 (Refuted)

**Statement:** Dark energy EOS w₀ = −5/6 ≈ −0.8333 emerges from twist-sector distribution in the exceptional framework.

**Status:** Refuted (`proofs/verified/C038_WDarkEnergy.v`).

The prediction w₀ = −5/6 is inconsistent with joint CMB + BAO + SNe Ia constraints, which place w₀ close to −1 (cosmological constant). The current best-fit value from multi-probe data is w₀ ≈ −0.95 ± 0.07, excluding w₀ = −5/6 at > 2σ.
...
where t is diffusion time and a, β are model-dependent parameters. The bigraph attachment sweep (`data/csv/c039_spectral_dimension_bigraph_curve.csv`) shows the D_s behaviour as a function of attachment probability.
...
2. **Dynamical mechanism for w₀:** The equation of state w₀ = −5/6 requires a Lagrangian whose stress-energy tensor gives this value. No such Lagrangian based on F4 twist sectors has been constructed.
3. **Barbero–Immirzi from first principles:** The LQG value γ ≈ 0.2375 should be derived from a computation in the exceptional framework, not inferred from proximity to 1/4.
