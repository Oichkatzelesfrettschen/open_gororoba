...
Updated 2026-02-09 after Sprint 25 completion.
**See also:** [`docs/ROADMAP.md`](ROADMAP.md) for full architecture and research plan.

## Current State (Sprint 25, 2026-02-09)

**Test count**: 2280 total (unit + integration + doc), 0 clippy warnings.
**Claims**: C-001..C-500 (500 total). **Insights**: I-001..I-017. **Binaries**: 40.

## Recent Sprint Summaries

- **S25**: Dim=512 census: 33 regimes, 214M triangles, 275s (C-495). Fano Residue
  Distribution Law: dim/16 per residue with deficit at 0 (C-496). Fano Projection Full
  Mixing: all components use all 7 Fano indices (C-497). Layer 3/4 bridge with budget-
  aware LightGraphInvariants O(n+e). Coset Obstruction Theorem: 0% Z/F_3 closure on
  Lambda_n explained by l_0=-1 coset mapping to forbidden l_0=+1 (C-498). Affine F_3
  Partial Closure 30-47% (C-499). Euclidean Filtration Ultrametricity Gradient: Lambda_256
  z=17.07 significant, Lambda_2048 anti-ultrametric, transition at Lambda_512 (C-500).
...
| M. Face sign census | DONE | S21-S24 | 33 regimes at dim=512, Double 3:1 Law (C-481..C-495) |
| N. Coset / filtration | DONE | S25 | Coset obstruction (C-498), filtration gradient (C-500) |
...
2) ~~**Primary-source citation for every claim** (ROADMAP 7.2)~~ -- **COMPLETE** (Sprint 25 audit).
   All 500/500 claims have non-empty where_stated: 118 test-function refs (23.6%),
   382 document/paper refs (76.4%). No remediation needed.
