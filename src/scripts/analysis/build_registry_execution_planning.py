#!/usr/bin/env python3
"""
Build canonical execution-planning lane strict TOML registries.

Compatibility note:
- Canonical script: src/scripts/analysis/build_registry_execution_planning.py
- Legacy script name retained for deprecated wave5-batch4 aliases

Legacy Wave 5 Batch 4 mapping:
...
# Canonical binary overrides for experiments that are intentionally declared
# in binaries.toml but may still carry empty legacy binary fields in source rows.
EXPERIMENT_BINARY_OVERRIDES: dict[str, str] = {
    "E-021": "tessarines-mixed-quaternions-census",
    "E-022": "albert-algebra-structure-census",
    "E-023": "composition-algebra-taxonomy",
    "E-024": "registry-event-tracker",
    "E-026": "third-party-source-verifier",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Build canonical execution-planning lane registries. "
            "Compatibility: legacy Wave 5 Batch 4 alias available via shim script."
        )
    )
    parser.add_argument(
        "--repo-root",
