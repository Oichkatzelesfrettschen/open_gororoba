#!/usr/bin/env python3
"""
Migrate CSV corpora into canonical TOML datasets.
...
DEFAULT_CANON_DIR = "registry/data/legacy_csv"
DEFAULT_INDEX_PATH = "registry/legacy_csv_datasets.toml"
DEFAULT_INDEX_TABLE = "legacy_csv_datasets"
DEFAULT_DATASET_PREFIX = "LC"
...
def _render_dataset(dataset: Dataset, corpus_label: str) -> str:
    lines: list[str] = []
    lines.append(f"# Canonical TOML dataset migrated from {corpus_label}.")
...
def _render_index(
    ...
    *,
    ...
    lines: list[str] = []
    lines.append(f"# Canonical index for {corpus_label} datasets migrated to TOML.")
    lines.append("# Generated by src/scripts/analysis/migrate_legacy_csv_to_toml.py")
    lines.append("")
    lines.append(f"[{index_table}]")
    lines.append(f'updated = "{UPDATED_STAMP}"')
    lines.append("authoritative = true")
    lines.append(f"source_glob = {_esc(source_glob)}")
    lines.append(f"canonical_dir = {_esc(canonical_dir)}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Glob for source CSV files.",
    )
    parser.add_argument(
        "--out-index",
    ...
    parser.add_argument(
        "--index-table",
        default=DEFAULT_INDEX_TABLE,
        help="Top-level TOML table name for aggregate index metadata.",
    )
    parser.add_argument(
        "--dataset-prefix",
        default=DEFAULT_DATASET_PREFIX,
        help="Dataset ID prefix (e.g. LC for legacy, CU for curated).",
    )
    parser.add_argument(
        "--corpus-label",
        default=DEFAULT_CORPUS_LABEL,
        help="Human-readable label used in generated comments.",
    )
    ...
    sources = sorted((root).glob(args.source_glob))
    if not sources:
        raise SystemExit(f"ERROR: no source CSV files found for glob {args.source_glob!r}")

    datasets: list[Dataset] = []
    dataset_prefix = args.dataset_prefix.strip().upper()
    if not re.fullmatch(r"[A-Z0-9]{1,6}", dataset_prefix):
        raise SystemExit("ERROR: --dataset-prefix must match [A-Z0-9]{1,6}")
    ...
    for idx, source in enumerate(sources, start=1):
        ...
        dataset_id = f"{dataset_prefix}-{idx:04d}"
    ...
    for dataset in datasets:
        ...
        text = _render_dataset(dataset, args.corpus_label)
