...
from repo_fastpath import worker_budget
...
SKIP_PATH_PARTS = {".cache", "cargo-home", "target", ".pytest_cache", "venv", ".venv", ".mamba"}
...
def _scan_toml_document(args: tuple[Path, str, str]) -> dict[str, object]:
    root, path, git_status = args
    full = root / path
    raw = full.read_bytes()
    ...
    try:
        ...
        authoritative = _has_authoritative(data)
...
def main() -> int:
    ...
    ignored = _git_paths(
        root,
        ["ls-files", "--others", "--ignored", "--exclude-standard", "*.toml"],
    )

    all_paths = sorted(path for path in (tracked | untracked | ignored) if not _should_skip(path))
    ...
    for path in all_paths:
        if path in tracked:
            ...
            git_status = "ignored"
        tasks.append((root, path, git_status))

    doc_rows: list[dict[str, object]] = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=worker_budget(root)) as executor:
        for row in executor.map(_scan_toml_document, tasks):
            if not bool(row["parse_ok"]):
                parse_error_paths.append(str(row["path"]))
            status_counts[str(row["git_status"])] += 1
            role_counts[str(row["role"])] += 1
            zone_counts[str(row["zone"])] += 1
            doc_rows.append(row)
