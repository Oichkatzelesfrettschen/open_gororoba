...
def _kind_for_path(path: str, text: str) -> tuple[str, str, bool]:
    if path in MIRROR_TO_TOML:
        return ("markdown_mirror", "manual", False)
...
def _render_toml(records: list[DocumentRecord]) -> str:
    ...
    lines.append("# Regenerate with: python3 src/scripts/analysis/build_knowledge_sources_registry.py")
    lines.append("")
    lines.append("[knowledge_sources]")
    lines.append(f"generated_at = {_escape_toml_string(now)}")
