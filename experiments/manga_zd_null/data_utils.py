...
def generate_manga_sample(
    ...
    Parameters:
        n_galaxies: number of galaxies (default 6992 to match E-183)
        seed: random seed for reproducibility
        x_min, x_max: radial range in units of r/r_s
        n_points_per_galaxy: average number of rotation curve points
        inject_alpha_zd: if > 0, inject a ZD harmonic signal at this amplitude
        cd_dim: Cayley-Dickson dimension for injection (default 16 = sedenion)

    Returns:
        list of SyntheticGalaxy objects
    """
    rng = np.random.RandomState(seed)

    # Mass distribution: log-normal centered at log(M200)=11.8, sigma=0.6
    # Calibrated to match MaNGA DR17 mass function
    log_m200_samples = rng.normal(11.8, 0.6, n_galaxies)
    log_m200_samples = np.clip(log_m200_samples, 10.2, 15.0)

    # Redshift distribution: narrow, z ~ 0.03 +/- 0.015 (MaNGA is low-z)
    z_samples = rng.normal(0.03, 0.015, n_galaxies)
    z_samples = np.clip(z_samples, 0.005, 0.15)

    # Inclination: uniform in cos(i) from 0 to 70 deg (MaNGA selection)
    ...
    for i in range(n_galaxies):
        ...
        # NFW parameters with scatter
        c200 = duffy08_concentration(log_m200, z)
        c200 *= rng.lognormal(0, 0.12)  # 12% log-normal scatter
        c200 = max(c200, 2.0)
        ...
        # Variable number of points per galaxy (Poisson around mean)
        n_pts = max(8, rng.poisson(n_points_per_galaxy))
        x_points = np.sort(rng.uniform(x_min, x_max, n_pts))

        # Compute NFW residuals with baryonic systematics + noise
        ...
        for j in range(n_pts):
            ...
            # Baryonic systematic (deterministic, same for all galaxies)
            sys = baryonic_systematic(x)

            # Galaxy-to-galaxy scatter: ~8% RMS (from E-183 measurement)
            scatter = rng.normal(0, 0.08)

            # Measurement noise: ~3-10% of v_circ depending on S/N
            _ = nfw_v_circ(r_kpc, r_s, m200, c200)
            meas_noise_frac = rng.uniform(0.03, 0.10)

            delta_v[j] = sys + scatter
            delta_v_err[j] = max(meas_noise_frac, 0.01)

            # Inject ZD signal if requested
...
def generate_leakage_grid(
    ...
    """
    Estimate expected DFT power under white noise on the actual data grid.

    Used to calibrate window function leakage: on a non-uniform, finite grid
    even pure white noise produces non-uniform power across wavenumbers.

    Returns:
        expected_power: shape (n_modes,) -- mean power per mode under H0.
    """
    rng = np.random.RandomState(seed)
    n_valid = len(x_valid)
    n_modes = len(wavenumbers)
    power_accum = np.zeros(n_modes)

    for _ in range(n_sim):
        noise = rng.normal(0.0, 1.0, n_valid)
        for i, k in enumerate(wavenumbers):
            re = np.sum(noise * np.cos(k * x_valid)) / n_valid
            im = np.sum(noise * np.sin(k * x_valid)) / n_valid
            power_accum[i] += re * re + im * im

    return power_accum / n_sim
